...
def main():
    ...
    parser.add_argument(
        "--campaign-path",
        required=True,
        help="Campaign path. E.g: C:\\ci\\CI 4x4 ORAN-FH.xml",
    )
    parser.add_argument(
        "--testname",
        required=True,
        help='Testname in the campaign. E.g: "32UE static DL + UL UDP - Dell"',
    )
    parser.add_argument(
        "--timeout", required=True, help="Timeout in seconds for the test"
    )
    parser.add_argument(
        "--loglevel",
        default="warning",
    ...
    MAKE_ARGS = "-j6"
    BUILD_ARGS = '-DCMAKE_BUILD_TYPE=Release -DFORCE_DEBUG_INFO=True -DENABLE_UHD=False -DENABLE_DPDK=True -DENABLE_ZEROMQ=False -DAUTO_DETECT_ISA=False -DCMAKE_CXX_FLAGS="-march=x86-64-v4"'
    DPDK_VERSION = "23.11_avx512"

    TESTBED = "viavi"
    MARKERS = "viavi_manual"

    PYARGS = f'--viavi-manual-campaign-filename "{campaign_name}" --viavi-manual-test-name "{test_name}" --viavi-manual-test-timeout {timeout} --retina-pod-timeout 900'
    ...
    variables = [
        ...
        {"key": "E2E_LOG_LEVEL", "value": loglevel},
