  ...
  void process_results()
  {
    ...
    if (res.success) {
      ...
      for (const auto& pucch : res.ul.pucchs) {
        ...
        if (pucch.get_format() == pucch_format::FORMAT_1) {
          uci_indication::uci_pdu::uci_pucch_f0_or_f1_pdu f1{};
          f1.sr_detected = false;
          if (pucch.get_harq_ack_nof_bits() > 0 and pucch.get_sr_bits() == sr_nof_bits::no_sr) {
            f1.ul_sinr_dB = 10;
            f1.harqs.resize(pucch.get_harq_ack_nof_bits(), mac_harq_ack_report_status::ack);
          } else if (pucch.get_harq_ack_nof_bits() > 0) {
            // ACK+SR
            f1.ul_sinr_dB = -10;
            f1.harqs.resize(pucch.get_harq_ack_nof_bits(), mac_harq_ack_report_status::dtx);
          } else {
            f1.ul_sinr_dB  = -10;
            f1.sr_detected = false;
          }
        ...
        } else if (pucch.get_format() == pucch_format::FORMAT_2) {
          uci_indication::uci_pdu::uci_pucch_f2_or_f3_or_f4_pdu f2{};
          f2.ul_sinr_dB = 10;
          if (pucch.get_harq_ack_nof_bits() > 0) {
            f2.harqs.resize(pucch.get_harq_ack_nof_bits(), mac_harq_ack_report_status::ack);
          }
