...
int main(int argc, char** argv)
{
  ...
  gw2_cfg.bind_port         = 56701;
  gw2_cfg.non_blocking_mode = false;
  gw2_cfg.rx_max_mmsg       = 256;

  dummy_network_gateway_data_notifier_with_src_addr gw2_dn{params};
  ...
  while (gw2_dn.get_n_pdus() < params.nof_pdus) {
  ...
  fmt::print("Rx data rate: {:.2f} Mbit/s\n\n",
             (double)gw2_dn.get_rx_bytes() * 8 * 1e-6 / (gw2_dn.get_t_sum().count() * 1e-6));
