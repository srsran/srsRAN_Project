...
TEST(mac_fapi_ul_pucch_format1_pdu_conversor_test, ul_pucch_format1_pdu_valid_should_pass)
{
  ...
  ASSERT_EQ(static_cast<unsigned>(mac_pdu.bits.sr_bits), fapi_pdu.sr_bit_len);
  ASSERT_EQ(mac_pdu.bits.harq_ack_nof_bits, fapi_pdu.bit_len_harq);
...
TEST(mac_fapi_ul_pucch_format2_pdu_conversor_test, ul_pucch_format2_pdu_valid_should_pass)
{
  ...
  ASSERT_EQ(static_cast<unsigned>(mac_pdu.bits.sr_bits), fapi_pdu.sr_bit_len);
  ASSERT_EQ(mac_pdu.bits.harq_ack_nof_bits, fapi_pdu.bit_len_harq);
  ASSERT_EQ(mac_pdu.bits.csi_part1_bits, fapi_pdu.csi_part1_bit_length);
}
