...
TEST_F(test_pucch_harq_allocator_ded_resources, test_sr_allocation_on_format2)
{
  ...
  ASSERT_EQ(1, slot_grid.result.ul.pucchs.size());
  ASSERT_EQ(srsran::sr_nof_bits::no_sr, slot_grid.result.ul.pucchs.back().format_2.sr_bits);
  ...
  ASSERT_EQ(srsran::sr_nof_bits::one, slot_grid.result.ul.pucchs.back().format_2.sr_bits);
}
...
TEST_F(test_pucch_harq_allocator_ded_resources, test_allocate_csi_over_f1_harq_2_ues)
{
  ...
  t_bench.add_ue();
  const optional<unsigned> ue_2_pucch_res_indicator =
      t_bench.pucch_alloc.alloc_ded_pucch_harq_ack_ue(t_bench.res_grid,
                                                      t_bench.last_allocated_rnti,
                                                      t_bench.get_ue(t_bench.last_allocated_ue_idx).get_pcell().cfg(),
                                                      t_bench.k0,
                                                      t_bench.k1);
  ASSERT_TRUE(ue_2_pucch_res_indicator.has_value());
  ASSERT_EQ(0, ue_2_pucch_res_indicator.value());

  // Allocate UE 1 a PUCCH resource (it'll get assigned a resource with PUCCH resource ind. = 1).
  const optional<unsigned> ue_1_pucch_res_indicator = t_bench.pucch_alloc.alloc_ded_pucch_harq_ack_ue(
      t_bench.res_grid, t_bench.get_main_ue().crnti, t_bench.get_main_ue().get_pcell().cfg(), t_bench.k0, t_bench.k1);
  const unsigned ue_1_pucch_f1_res_indicator = 1;
  ASSERT_TRUE(ue_1_pucch_res_indicator.value());
  ASSERT_EQ(ue_1_pucch_f1_res_indicator, ue_1_pucch_res_indicator.value());
...
TEST_F(test_pucch_harq_allocator_ded_resources, test_res_indicator_harq_after_csi)
{
  ...
  ASSERT_TRUE(test_pucch_res_indicator.has_value());
  ASSERT_EQ(2, test_pucch_res_indicator.value());
  ASSERT_EQ(pucch_format::FORMAT_2, slot_grid.result.ul.pucchs.back().format);
}
...
TEST_F(test_pucch_harq_allocator_ded_resources, test_harq_ack_alloc_with_csi_fails_when_exceeding_payload)
{
  ...
  const optional<unsigned> pucch_res_indicator = t_bench.pucch_alloc.alloc_ded_pucch_harq_ack_ue(
      t_bench.res_grid, t_bench.get_main_ue().crnti, t_bench.get_main_ue().get_pcell().cfg(), t_bench.k0, k1);
  ...
  // extra HARQ-ACK bit).
  const optional<unsigned> pucch_res_ind_2nd_alloc = t_bench.pucch_alloc.alloc_ded_pucch_harq_ack_ue(
      t_bench.res_grid, t_bench.get_main_ue().crnti, t_bench.get_main_ue().get_pcell().cfg(), t_bench.k0, k1);
  ASSERT_TRUE(pucch_res_ind_2nd_alloc.has_value());
  ...
  const optional<unsigned> pucch_res_ind_3rd_alloc = t_bench.pucch_alloc.alloc_ded_pucch_harq_ack_ue(
      t_bench.res_grid, t_bench.get_main_ue().crnti, t_bench.get_main_ue().get_pcell().cfg(), t_bench.k0, k1);
  ASSERT_TRUE(pucch_res_ind_3rd_alloc.has_value());
  ASSERT_EQ(3, slot_grid.result.ul.pucchs.back().format_2.harq_ack_nof_bits);

  // Expect the fourth HARQ-ACK grant allocation to succeed (it should be the same grant as the previous one, with one
  // extra HARQ-ACK bit).
  const optional<unsigned> pucch_res_ind_4th_alloc = t_bench.pucch_alloc.alloc_ded_pucch_harq_ack_ue(
      t_bench.res_grid, t_bench.get_main_ue().crnti, t_bench.get_main_ue().get_pcell().cfg(), t_bench.k0, k1);
  ASSERT_TRUE(pucch_res_ind_4th_alloc.has_value());
  ASSERT_EQ(4, slot_grid.result.ul.pucchs.back().format_2.harq_ack_nof_bits);

  // Expect the fifth HARQ-ACK grant allocation to fail, as it falls on a CSI slot (4 bits) and the max payload has been
  // reached.
  const optional<unsigned> pucch_res_ind_5th_alloc = t_bench.pucch_alloc.alloc_ded_pucch_harq_ack_ue(
      t_bench.res_grid, t_bench.get_main_ue().crnti, t_bench.get_main_ue().get_pcell().cfg(), t_bench.k0, k1);
  ASSERT_FALSE(pucch_res_ind_5th_alloc.has_value());
}
...
TEST_F(test_pucch_harq_allocator_ded_resources, test_harq_ack_alloc_with_sr_fails_when_exceeding_payload)
{
  ...
  // reached.
  const optional<unsigned> pucch_res_indicator_8th_alloc = t_bench.pucch_alloc.alloc_ded_pucch_harq_ack_ue(
      t_bench.res_grid, t_bench.get_main_ue().crnti, t_bench.get_main_ue().get_pcell().cfg(), t_bench.k0, k1);
  ASSERT_FALSE(pucch_res_indicator_8th_alloc.has_value());
}
