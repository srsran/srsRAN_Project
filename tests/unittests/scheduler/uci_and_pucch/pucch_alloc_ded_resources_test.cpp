...
public:
  test_pucch_allocator_ded_resources()
  {
    ...
    pucch_expected_f1_sr.bits.sr_bits              = sr_nof_bits::one;
    pucch_expected_f1_sr.bits.harq_ack_nof_bits    = 0;
    ...
    pucch_expected_f1_harq.bits.sr_bits              = sr_nof_bits::no_sr;
    pucch_expected_f1_harq.bits.harq_ack_nof_bits    = 1;
...
TEST_F(test_pucch_allocator_ded_resources, test_csi_alloc_only)
{
  ...
  pucch_expected_csi.bits.harq_ack_nof_bits = 0;
  pucch_expected_csi.bits.sr_bits           = sr_nof_bits::no_sr;
  pucch_expected_csi.bits.csi_part1_bits    = csi_part1_bits;
...
TEST_F(test_pucch_allocator_ded_resources, test_csi_alloc_over_sr)
{
  const unsigned csi_part1_bits             = 4;
  pucch_expected_csi.bits.harq_ack_nof_bits = 0;
  pucch_expected_csi.bits.sr_bits           = sr_nof_bits::one;
  pucch_expected_csi.bits.csi_part1_bits    = csi_part1_bits;
...
{
  pucch_expected_f1_sr.bits.harq_ack_nof_bits = 1;
...
{
  pucch_expected_f1_harq.bits.harq_ack_nof_bits = 2;
...
{
  pucch_expected_f1_harq.bits.harq_ack_nof_bits = 2;
  pucch_expected_f1_sr.bits.harq_ack_nof_bits   = 2;
  ...
  const auto* first_alloc = std::find_if(slot_grid.result.ul.pucchs.begin(),
                                         ...
                                         [rnti = t_bench.get_main_ue().crnti](const pucch_info& pdu) {
                                           return pdu.format() == pucch_format::FORMAT_1 and pdu.crnti == rnti and
                                                  pdu.bits.harq_ack_nof_bits == 1U and
                                                  pdu.bits.sr_bits == sr_nof_bits::no_sr;
                                         });
  ...
  const auto* second_alloc = std::find_if(slot_grid.result.ul.pucchs.begin(),
                                          ...
                                          [rnti = t_bench.get_main_ue().crnti](const pucch_info& pdu) {
                                            return pdu.format() == pucch_format::FORMAT_1 and pdu.crnti == rnti and
                                                   pdu.bits.harq_ack_nof_bits == 2U and
                                                   pdu.bits.sr_bits == sr_nof_bits::no_sr;
                                          });
  ASSERT_TRUE(first_alloc != slot_grid.result.ul.pucchs.end());
  ASSERT_EQ(first_alloc->resources.prbs, second_alloc->resources.prbs);
...
TEST_F(test_pucch_allocator_ded_resources, test_harq_alloc_3bits)
{
  ...
  ASSERT_EQ(1, slot_grid.result.ul.pucchs[0].bits.harq_ack_nof_bits);
  ...
  ASSERT_EQ(1, slot_grid.result.ul.pucchs.size());
  ASSERT_EQ(pucch_format::FORMAT_1, slot_grid.result.ul.pucchs[0].format());
  ASSERT_EQ(2, slot_grid.result.ul.pucchs[0].bits.harq_ack_nof_bits);
  ...
  ASSERT_EQ(3, slot_grid.result.ul.pucchs[0].bits.harq_ack_nof_bits);
}

TEST_F(test_pucch_allocator_ded_resources, test_harq_alloc_3bits_over_sr)
{
  pucch_expected_f2.bits.harq_ack_nof_bits = 3;
  pucch_expected_f2.bits.sr_bits           = sr_nof_bits::one;
  pucch_expected_f2.bits.csi_part1_bits    = 0;
...
TEST_F(test_pucch_allocator_ded_resources, test_harq_alloc_1bit_over_csi)
{
  ...
  pucch_expected_f2.bits.harq_ack_nof_bits = 1;
  pucch_expected_f2.bits.sr_bits           = sr_nof_bits::no_sr;
  pucch_expected_f2.bits.csi_part1_bits    = 4;
...
TEST_F(test_pucch_allocator_ded_resources, test_harq_alloc_2bits_over_csi)
{
  ...
  // multiplexed into 1, which is the PUCCH HARQ resource from set 1 (Format 2).
  pucch_expected_f2.bits.harq_ack_nof_bits = 2;
  pucch_expected_f2.bits.sr_bits           = sr_nof_bits::no_sr;
  pucch_expected_f2.bits.csi_part1_bits    = 4;
...
TEST_F(test_pucch_allocator_ded_resources, test_harq_alloc_3bits_over_csi)
{
  ...
  pucch_expected_csi.bits.harq_ack_nof_bits = 0;
  pucch_expected_csi.bits.sr_bits           = sr_nof_bits::no_sr;
  pucch_expected_csi.bits.csi_part1_bits    = 4;

  pucch_info pucch_f2_harq_csi_mplexed             = pucch_expected_f2;
  pucch_f2_harq_csi_mplexed.bits.harq_ack_nof_bits = 3;
  pucch_f2_harq_csi_mplexed.bits.sr_bits           = sr_nof_bits::no_sr;
  pucch_f2_harq_csi_mplexed.bits.csi_part1_bits    = 4;
...
TEST_F(test_pucch_allocator_ded_resources, test_harq_alloc_4bits_over_sr_and_csi)
{
  // With SR and with PUCCH Format 1 it is guaranteed that the resources will be multiplexed, as PUCCH Format 1 for SR
  // spans over the 14 symbols.
  pucch_expected_f2.bits.harq_ack_nof_bits = 4;
  pucch_expected_f2.bits.sr_bits           = sr_nof_bits::one;
  pucch_expected_f2.bits.csi_part1_bits    = 3;
...
TEST_F(test_pucch_allocator_ded_resources, test_common_plus_ded_resource_without_existing_grants)
{
  ...
  ASSERT_EQ(pucch_format::FORMAT_1, slot_grid.result.ul.pucchs[0].format());
  ASSERT_EQ(1, slot_grid.result.ul.pucchs[0].bits.harq_ack_nof_bits);
  ASSERT_EQ(sr_nof_bits::no_sr, slot_grid.result.ul.pucchs[0].bits.sr_bits);
  ASSERT_TRUE(find_pucch_pdu(slot_grid.result.ul.pucchs, [&expected = pucch_expected_f1_harq](const pucch_info& pdu) {
    return pucch_info_match(expected, pdu);
  }));
  // PUCCH common resource.
  ASSERT_EQ(pucch_format::FORMAT_1, slot_grid.result.ul.pucchs[1].format());
  ASSERT_EQ(1, slot_grid.result.ul.pucchs[1].bits.harq_ack_nof_bits);
  ASSERT_EQ(sr_nof_bits::no_sr, slot_grid.result.ul.pucchs[1].bits.sr_bits);
...
{
  pucch_expected_f1_harq.bits.harq_ack_nof_bits = 1U;
  pucch_expected_f1_harq.bits.sr_bits           = sr_nof_bits::no_sr;

  pucch_expected_f1_sr.bits.harq_ack_nof_bits = 1U;
  pucch_expected_f1_sr.bits.sr_bits           = sr_nof_bits::one;
  ...
  ASSERT_TRUE(find_pucch_pdu(pucch_pdus, [](const pucch_info& pdu) {
    return pdu.format() == pucch_format::FORMAT_1 and pdu.bits.sr_bits == sr_nof_bits::no_sr and
           pdu.bits.harq_ack_nof_bits == 1U and pdu.pdu_context.is_common;
  }));
}

TEST_F(test_pucch_allocator_ded_resources, test_common_plus_ded_resource_with_existing_f2_csi)
{
  pucch_expected_f2.bits.harq_ack_nof_bits = 1U;
  pucch_expected_f2.bits.sr_bits           = srsran::sr_nof_bits::no_sr;
  pucch_expected_f2.bits.csi_part1_bits    = 4U;
...
TEST_F(test_pucch_allocator_ded_resources, test_harq_f2_alloc_2_ues)
{
  ...
  ASSERT_EQ(2, slot_grid.result.ul.pucchs.size());
  ASSERT_EQ(pucch_format::FORMAT_2, slot_grid.result.ul.pucchs[0].format());
  ASSERT_EQ(3, slot_grid.result.ul.pucchs.back().bits.harq_ack_nof_bits);
}
...
TEST_F(test_pucch_allocator_ded_resources, test_multiple_ues_and_csi_alloc)
{
  ...
  ASSERT_EQ(1, slot_grid.result.ul.pucchs.size());
  ASSERT_EQ(pucch_format::FORMAT_2, slot_grid.result.ul.pucchs.back().format());
  ASSERT_EQ(4, slot_grid.result.ul.pucchs.back().bits.csi_part1_bits);
...
TEST_F(test_pucch_allocator_ded_resources, test_harq_sr_f2_alloc_multiple_ues)
{
  ...
  ASSERT_EQ(pucch_format::FORMAT_2, slot_grid.result.ul.pucchs.back().format());
  ASSERT_EQ(3, slot_grid.result.ul.pucchs.back().bits.harq_ack_nof_bits);
  ASSERT_EQ(sr_nof_bits::one, slot_grid.result.ul.pucchs.back().bits.sr_bits);
}
...
TEST_F(test_pucch_allocator_ded_resources, test_tdd_harq_allocation_over_time)
{
  ...
  ASSERT_EQ(3, slot_grid_3.result.ul.pucchs.back().bits.harq_ack_nof_bits);
  ...
  ASSERT_EQ(4, slot_grid_4.result.ul.pucchs.back().bits.harq_ack_nof_bits);
  ...
  ASSERT_EQ(5, slot_grid_5.result.ul.pucchs.back().bits.harq_ack_nof_bits);
}
...
TEST_F(test_pucch_allocator_ded_resources, test_for_private_fnc_retrieving_existing_grants)
{
  ...
  ASSERT_TRUE(find_pucch_pdu(slot_grid.result.ul.pucchs, [rnti = t_bench.get_ue(ue1_idx).crnti](const pucch_info& pdu) {
    return pdu.format() == pucch_format::FORMAT_1 and pdu.crnti == rnti and pdu.bits.harq_ack_nof_bits == 1U;
  }));
  ...
  ASSERT_TRUE(find_pucch_pdu(slot_grid.result.ul.pucchs, [rnti = t_bench.get_main_ue().crnti](const pucch_info& pdu) {
    return pdu.format() == pucch_format::FORMAT_1 and pdu.crnti == rnti and pdu.bits.harq_ack_nof_bits == 1U and
  ...
  ASSERT_EQ(3, slot_grid.result.ul.pucchs.size());
  ASSERT_TRUE(find_pucch_pdu(slot_grid.result.ul.pucchs, [rnti = t_bench.get_ue(ue1_idx).crnti](const pucch_info& pdu) {
    return pdu.format() == pucch_format::FORMAT_1 and pdu.crnti == rnti and pdu.bits.harq_ack_nof_bits == 2U;
  }));
  ASSERT_TRUE(find_pucch_pdu(slot_grid.result.ul.pucchs, [rnti = t_bench.get_main_ue().crnti](const pucch_info& pdu) {
    return pdu.format() == pucch_format::FORMAT_1 and pdu.crnti == rnti and pdu.bits.harq_ack_nof_bits == 1U and
           pdu.pdu_context.is_common;
  ...
  ASSERT_EQ(3, slot_grid.result.ul.pucchs.size());
  ASSERT_TRUE(find_pucch_pdu(slot_grid.result.ul.pucchs, [rnti = t_bench.get_ue(ue1_idx).crnti](const pucch_info& pdu) {
    return pdu.format() == pucch_format::FORMAT_2 and pdu.crnti == rnti and pdu.bits.harq_ack_nof_bits == 3U;
  }));
  ASSERT_TRUE(find_pucch_pdu(slot_grid.result.ul.pucchs, [rnti = t_bench.get_main_ue().crnti](const pucch_info& pdu) {
    return pdu.format() == pucch_format::FORMAT_1 and pdu.crnti == rnti and pdu.bits.harq_ack_nof_bits == 1U and
           pdu.pdu_context.is_common;
