...
{
public:
  test_pucch_allocator_ded_resources(){// Set expected grant for PUCCH Format 1 SR.
                                       {auto& format_1 = pucch_expected_f1_sr.format_params.emplace<pucch_format_1>();
...
{
  auto& format_1                 = pucch_expected_f1_harq.format_params.emplace<pucch_format_1>();
...
TEST_F(test_pucch_allocator_ded_resources, test_csi_alloc_only)
{
  ...
  auto&          format_2       = std::get<pucch_format_2>(pucch_expected_csi.format_params);
...
{
  auto& format_1             = std::get<pucch_format_1>(pucch_expected_f1_sr.format_params);
  format_1.harq_ack_nof_bits = 1;
...
{
  auto& format_1             = std::get<pucch_format_1>(pucch_expected_f1_harq.format_params);
...
{
  auto& harq_format_1             = std::get<pucch_format_1>(pucch_expected_f1_harq.format_params);
  harq_format_1.harq_ack_nof_bits = 2;
  auto& sr_format_1               = std::get<pucch_format_1>(pucch_expected_f1_sr.format_params);
  sr_format_1.harq_ack_nof_bits   = 2;
  ...
  const auto* first_alloc = std::find_if(slot_grid.result.ul.pucchs.begin(),
                                         ...
                                         [rnti = t_bench.get_main_ue().crnti](const pucch_info& pdu) {
                                           return pdu.get_format() == pucch_format::FORMAT_1 and pdu.crnti == rnti and
                                                  pdu.get_harq_ack_nof_bits() == 1U and
  ...
  const auto* second_alloc = std::find_if(slot_grid.result.ul.pucchs.begin(),
                                          ...
                                          [rnti = t_bench.get_main_ue().crnti](const pucch_info& pdu) {
                                            return pdu.get_format() == pucch_format::FORMAT_1 and pdu.crnti == rnti and
                                                   pdu.get_harq_ack_nof_bits() == 2U and
                                                   pdu.get_sr_bits() == sr_nof_bits::no_sr;
                                          });
  ASSERT_TRUE(first_alloc != slot_grid.result.ul.pucchs.end());
  ASSERT_EQ(first_alloc->resources.prbs, second_alloc->resources.prbs);
...
TEST_F(test_pucch_allocator_ded_resources, test_harq_alloc_3bits)
{
  ...
  ASSERT_EQ(pucch_res_idx, test_pucch_res_indicator.value());
  ASSERT_EQ(1, slot_grid.result.ul.pucchs.size());
  ASSERT_EQ(pucch_format::FORMAT_1, slot_grid.result.ul.pucchs[0].get_format());
  ASSERT_EQ(1, slot_grid.result.ul.pucchs[0].get_harq_ack_nof_bits());
  ...
  ASSERT_EQ(1, slot_grid.result.ul.pucchs.size());
  ASSERT_EQ(pucch_format::FORMAT_1, slot_grid.result.ul.pucchs[0].get_format());
  ASSERT_EQ(2, slot_grid.result.ul.pucchs[0].get_harq_ack_nof_bits());
  ...
  ASSERT_EQ(pucch_res_idx, test_pucch_res_indicator.value());
  ASSERT_EQ(1, slot_grid.result.ul.pucchs.size());
  ASSERT_EQ(pucch_format::FORMAT_2, slot_grid.result.ul.pucchs[0].get_format());
  ASSERT_EQ(3, slot_grid.result.ul.pucchs[0].get_harq_ack_nof_bits());
}

TEST_F(test_pucch_allocator_ded_resources, test_harq_alloc_3bits_over_sr)
{
  auto& format_2             = std::get<pucch_format_2>(pucch_expected_f2.format_params);
  format_2.harq_ack_nof_bits = 3;
  format_2.sr_bits           = sr_nof_bits::one;
...
TEST_F(test_pucch_allocator_ded_resources, test_harq_alloc_3bits_over_csi)
{
  ...
  auto& harq_format_2             = std::get<pucch_format_2>(pucch_expected_f2.format_params);
  harq_format_2.harq_ack_nof_bits = 3;
  harq_format_2.sr_bits           = sr_nof_bits::no_sr;
  harq_format_2.csi_part1_bits    = 0;

  auto& csi_format_2             = std::get<pucch_format_2>(pucch_expected_csi.format_params);
  csi_format_2.harq_ack_nof_bits = 0;
  csi_format_2.sr_bits           = sr_nof_bits::no_sr;
  csi_format_2.csi_part1_bits    = 4;

  pucch_info pucch_f2_harq_csi_mplexed = pucch_expected_f2;
  auto&      mplexed_format_2          = std::get<pucch_format_2>(pucch_f2_harq_csi_mplexed.format_params);
  mplexed_format_2.harq_ack_nof_bits   = 3;
  mplexed_format_2.sr_bits             = sr_nof_bits::no_sr;
  mplexed_format_2.csi_part1_bits      = 4;
...
TEST_F(test_pucch_allocator_ded_resources, test_harq_alloc_4bits_over_sr_and_csi)
{
  ...
  // spans over the 14 symbols.
  auto& format_2             = std::get<pucch_format_2>(pucch_expected_f2.format_params);
  format_2.harq_ack_nof_bits = 4;
  format_2.sr_bits           = sr_nof_bits::one;
  format_2.csi_part1_bits    = 3;
...
TEST_F(test_pucch_allocator_ded_resources, test_common_plus_ded_resource_without_existing_grants)
{
  ...
  ASSERT_EQ(pucch_format::FORMAT_1, slot_grid.result.ul.pucchs[0].get_format());
  ASSERT_EQ(1, slot_grid.result.ul.pucchs[0].get_harq_ack_nof_bits());
  ASSERT_EQ(sr_nof_bits::no_sr, slot_grid.result.ul.pucchs[0].get_sr_bits());
  ASSERT_TRUE(find_pucch_pdu(slot_grid.result.ul.pucchs, [&expected = pucch_expected_f1_harq](const pucch_info& pdu) {
    return pucch_info_match(expected, pdu);
  }));
  // PUCCH common resource.
  ASSERT_EQ(pucch_format::FORMAT_1, slot_grid.result.ul.pucchs[1].get_format());
  ASSERT_EQ(1, slot_grid.result.ul.pucchs[1].get_harq_ack_nof_bits());
  ASSERT_EQ(sr_nof_bits::no_sr, slot_grid.result.ul.pucchs[1].get_sr_bits());
...
{
  auto& harq_format_1             = std::get<pucch_format_1>(pucch_expected_f1_harq.format_params);
  harq_format_1.harq_ack_nof_bits = 1U;
  harq_format_1.sr_bits           = sr_nof_bits::no_sr;

  auto& sr_format_1             = std::get<pucch_format_1>(pucch_expected_f1_sr.format_params);
  sr_format_1.harq_ack_nof_bits = 1U;
  sr_format_1.sr_bits           = sr_nof_bits::one;
  ...
  ASSERT_TRUE(std::all_of(pucch_pdus.begin(), pucch_pdus.end(), [](const pucch_info& pdu) {
    return pdu.get_format() == pucch_format::FORMAT_1;
  }));
  ...
  ASSERT_TRUE(find_pucch_pdu(pucch_pdus, [](const pucch_info& pdu) {
    return pdu.get_format() == pucch_format::FORMAT_1 and pdu.get_sr_bits() == sr_nof_bits::no_sr and
           pdu.get_harq_ack_nof_bits() == 1U and pdu.pdu_context.is_common;
  }));
}
...
{
  auto& format_2             = std::get<pucch_format_2>(pucch_expected_f2.format_params);
  format_2.harq_ack_nof_bits = 1U;
  format_2.sr_bits           = srsran::sr_nof_bits::no_sr;
  format_2.csi_part1_bits    = 4U;
...
TEST_F(test_pucch_allocator_ded_resources, test_harq_f2_alloc_2_ues)
{
  ...
  ASSERT_EQ(2, slot_grid.result.ul.pucchs.size());
  ASSERT_EQ(pucch_format::FORMAT_2, slot_grid.result.ul.pucchs[0].get_format());
  ASSERT_EQ(3, slot_grid.result.ul.pucchs.back().get_harq_ack_nof_bits());
}
...
TEST_F(test_pucch_allocator_ded_resources, test_harq_f2_alloc_multiple_ues)
{
  ...
  ASSERT_EQ(pucch_format::FORMAT_2, slot_grid.result.ul.pucchs.back().get_format());
  ASSERT_EQ(3, slot_grid.result.ul.pucchs.back().get_harq_ack_nof_bits());
}
...
TEST_F(test_pucch_allocator_ded_resources, test_harq_f2_alloc_last_ue_not_allocated)
{
  ...
  // resources.
  ASSERT_EQ(7, slot_grid.result.ul.pucchs.size());
  ASSERT_EQ(pucch_format::FORMAT_1, slot_grid.result.ul.pucchs.back().get_format());
  ASSERT_FALSE(test_pucch_res_indicator.has_value());
}
...
TEST_F(test_pucch_allocator_ded_resources, test_multiple_ues_and_csi_alloc)
{
  ...
  ASSERT_EQ(1, slot_grid.result.ul.pucchs.size());
  ASSERT_EQ(pucch_format::FORMAT_2, slot_grid.result.ul.pucchs.back().get_format());
  ASSERT_EQ(4, slot_grid.result.ul.pucchs.back().get_csi_part1_bits());
...
TEST_F(test_pucch_allocator_ded_resources, test_harq_sr_f2_alloc_multiple_ues)
{
  ...
  ASSERT_EQ(pucch_format::FORMAT_2, slot_grid.result.ul.pucchs.back().get_format());
  ASSERT_EQ(3, slot_grid.result.ul.pucchs.back().get_harq_ack_nof_bits());
  ASSERT_EQ(sr_nof_bits::one, slot_grid.result.ul.pucchs.back().get_sr_bits());
}
...
TEST_F(test_pucch_allocator_ded_resources, test_tdd_harq_allocation_over_time)
{
  ...
  ASSERT_EQ(pucch_format::FORMAT_1, slot_grid_1.result.ul.pucchs.back().get_format());
  ...
  ASSERT_EQ(pucch_format::FORMAT_1, slot_grid_2.result.ul.pucchs.back().get_format());
  ...
  ASSERT_EQ(pucch_format::FORMAT_2, slot_grid_3.result.ul.pucchs.back().get_format());
  ASSERT_EQ(3, slot_grid_3.result.ul.pucchs.back().get_harq_ack_nof_bits());
  ...
  ASSERT_TRUE(test_pucch_res_indicator.has_value());
  ASSERT_EQ(0, test_pucch_res_indicator.value());
  ASSERT_EQ(pucch_format::FORMAT_2, slot_grid_4.result.ul.pucchs.back().get_format());
  ASSERT_EQ(4, slot_grid_4.result.ul.pucchs.back().get_harq_ack_nof_bits());
  ...
  ASSERT_TRUE(test_pucch_res_indicator.has_value());
  ASSERT_EQ(0, test_pucch_res_indicator.value());
  ASSERT_EQ(pucch_format::FORMAT_2, slot_grid_5.result.ul.pucchs.back().get_format());
  ASSERT_EQ(5, slot_grid_5.result.ul.pucchs.back().get_harq_ack_nof_bits());
}
...
TEST_F(test_pucch_allocator_ded_resources, test_for_private_fnc_retrieving_existing_grants)
{
  ...
  ASSERT_TRUE(find_pucch_pdu(slot_grid.result.ul.pucchs, [rnti = t_bench.get_ue(ue1_idx).crnti](const pucch_info& pdu) {
    return pdu.get_format() == pucch_format::FORMAT_1 and pdu.crnti == rnti and pdu.get_harq_ack_nof_bits() == 1U;
  }));
  ...
  ASSERT_TRUE(find_pucch_pdu(slot_grid.result.ul.pucchs, [rnti = t_bench.get_main_ue().crnti](const pucch_info& pdu) {
    return pdu.get_format() == pucch_format::FORMAT_1 and pdu.crnti == rnti and pdu.get_harq_ack_nof_bits() == 1U and
  ...
  ASSERT_EQ(3, slot_grid.result.ul.pucchs.size());
  ASSERT_TRUE(find_pucch_pdu(slot_grid.result.ul.pucchs, [rnti = t_bench.get_ue(ue1_idx).crnti](const pucch_info& pdu) {
    return pdu.get_format() == pucch_format::FORMAT_1 and pdu.crnti == rnti and pdu.get_harq_ack_nof_bits() == 2U;
  }));
  ASSERT_TRUE(find_pucch_pdu(slot_grid.result.ul.pucchs, [rnti = t_bench.get_main_ue().crnti](const pucch_info& pdu) {
    return pdu.get_format() == pucch_format::FORMAT_1 and pdu.crnti == rnti and pdu.get_harq_ack_nof_bits() == 1U and
           pdu.pdu_context.is_common;
  ...
  ASSERT_EQ(3, slot_grid.result.ul.pucchs.size());
  ASSERT_TRUE(find_pucch_pdu(slot_grid.result.ul.pucchs, [rnti = t_bench.get_ue(ue1_idx).crnti](const pucch_info& pdu) {
    return pdu.get_format() == pucch_format::FORMAT_2 and pdu.crnti == rnti and pdu.get_harq_ack_nof_bits() == 3U;
  }));
  ASSERT_TRUE(find_pucch_pdu(slot_grid.result.ul.pucchs, [rnti = t_bench.get_main_ue().crnti](const pucch_info& pdu) {
    return pdu.get_format() == pucch_format::FORMAT_1 and pdu.crnti == rnti and pdu.get_harq_ack_nof_bits() == 1U and
           pdu.pdu_context.is_common;
