       ...
       when_conres_ce_and_srb_pdu_are_enqueued_then_tc_rnti_is_used_and_multiplexing_with_csi_rs_is_avoided)
{
  ...
  // Ensure the ConRes CE is scheduled first.
  ASSERT_TRUE(this->run_slot_until(
      [this]() { return find_ue_pdsch(rnti, *this->last_sched_res_list[to_du_cell_index(0)]) != nullptr; }));
  ...
  ASSERT_EQ(msg4_alloc->tb_list[0].lc_chs_to_sched.size(), 1);
  ...
  // Then, SRB0/SRB1 is scheduled.
  ASSERT_TRUE(this->run_slot_until(
      [this]() { return find_ue_pdsch(rnti, *this->last_sched_res_list[to_du_cell_index(0)]) != nullptr; }));
  msg4_alloc = find_ue_pdsch(rnti, *this->last_sched_res_list[to_du_cell_index(0)]);
  ASSERT_EQ(msg4_alloc->tb_list.size(), 1);
  ASSERT_EQ(msg4_alloc->tb_list[0].lc_chs_to_sched.size(), 1);
  ASSERT_EQ(msg4_alloc->tb_list[0].lc_chs_to_sched[0].lcid, params.msg4_lcid);
  ...
  if (params.msg4_lcid == LCID_SRB0) {
    ASSERT_GE(msg4_alloc->tb_list[0].lc_chs_to_sched[0].sched_bytes, msg4_size);
  }
  // Ensure C-RNTI is used for Msg4 (Reestablishment).
  ASSERT_EQ(find_ue_dl_pdcch(rnti)->dci.type, dci_dl_rnti_config_type::c_rnti_f1_0);
