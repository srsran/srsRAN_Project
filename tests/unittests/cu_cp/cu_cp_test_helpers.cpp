...
cu_cp_test::cu_cp_test()
{
  ...
  e1ap_gw.attach_cu_cp_cu_up_repo(cu_cp_obj->get_cu_ups());
}
                                 ...
                                 gnb_cu_ue_f1ap_id_t cu_ue_id)
{
  ...
  cu_cp_obj->get_dus().get_du(du_index).get_f1ap_message_handler().handle_message(ul_rrc_msg_transfer);
...
void cu_cp_test::test_du_attach(du_index_t du_index, gnb_du_id_t gnb_du_id, unsigned nrcell_id, pci_t pci)
{
  ...
  cu_cp_obj->get_dus().get_du(du_index).get_f1ap_message_handler().handle_message(f1setup_msg);
}
