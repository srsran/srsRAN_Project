...
void synchronized_mock_cu_up::handle_rx_pdu(const e1ap_message& pdu)
{
  report_fatal_error_if_not(rx_pdus.push_blocking(pdu), "queue is full");
}

bool synchronized_mock_cu_up::request_new_cu_up_connection()
{
...
bool synchronized_mock_cu_up::try_pop_rx_pdu(e1ap_message& pdu)
{
