...
{
public:
  cu_cp_paging_test() : cu_cp_test_environment(cu_cp_test_env_params{8, 8, 8192, create_mock_amf()})
  {
  ...
  unsigned setup_du(const f1ap_message& f1_setup_request)
  {
    // Setup DU
  ...
  void send_ngap_paging(unsigned du_idx_, const ngap_message& paging_msg)
  {
    srsran_assert(not this->get_amf().try_pop_rx_pdu(ngap_pdu), "there are still NGAP messages to pop from AMF");
    srsran_assert(not this->get_du(du_idx_).try_pop_dl_pdu(f1ap_pdu),
                  "there are still F1AP DL messages to pop from DU");

    get_amf().push_tx_pdu(paging_msg);
  }

  void send_minimal_ngap_paging_and_await_f1ap_paging(unsigned du_idx_)
  {
    // Inject NGAP Paging and wait for F1AP Paging
    send_ngap_paging(du_idx_, generate_valid_minimal_paging_message());
    bool result = this->wait_for_f1ap_tx_pdu(du_idx_, f1ap_pdu);
    report_fatal_error_if_not(result, "Failed to receive F1AP Paging");
  ...
  void send_ngap_paging_and_await_f1ap_paging(unsigned du_idx_)
  {
    // Inject NGAP Paging and wait for F1AP Paging
    send_ngap_paging(du_idx_, generate_valid_paging_message());
    bool result = this->wait_for_f1ap_tx_pdu(du_idx_, f1ap_pdu);
    report_fatal_error_if_not(result, "Failed to receive F1AP Paging");
    report_fatal_error_if_not(test_helpers::is_valid_paging(f1ap_pdu), "Invalid F1AP Paging");
  ...
  bool is_valid_minimal_paging_result(const f1ap_message& msg)
  {
  ...
  bool is_valid_paging_result(const f1ap_message& msg)
  {
...
TEST_F(cu_cp_paging_test, when_du_connection_not_finished_then_paging_is_not_sent_to_du)
{
  ...
  send_ngap_paging(du_idx, generate_valid_minimal_paging_message());

  // Make sure that no paging was sent to the DU
  srsran_assert(not this->get_du(du_idx).try_pop_dl_pdu(f1ap_pdu), "Paging was sent to DU before F1Setup");
}
...
TEST_F(cu_cp_paging_test, when_no_du_for_tac_exists_then_paging_is_not_sent_to_du)
{
  ...
  send_ngap_paging(du_idx, paging_msg);

  // Make sure that no paging was sent to the DU
  srsran_assert(not this->get_du(du_idx).try_pop_dl_pdu(f1ap_pdu), "Paging with unknown TAC was sent to DU");
}
...
TEST_F(cu_cp_paging_test, when_assist_data_for_paging_for_unknown_tac_is_included_then_paging_is_not_sent_to_du)
{
  ...
  // Make sure that no paging was sent to the DU
  srsran_assert(not this->get_du(du_idx).try_pop_dl_pdu(f1ap_pdu),
                "Paging with unknown TAC but assist data was sent to DU");
}
...
TEST_F(cu_cp_paging_test, when_invalid_paging_message_received_then_paging_is_not_sent_to_du)
{
  ...
  send_ngap_paging(du_idx, generate_invalid_paging_message());

  // Make sure that no paging was sent to the DU
  srsran_assert(not this->get_du(du_idx).try_pop_dl_pdu(f1ap_pdu), "Invalid Paging was sent to DU");
}
...
TEST_F(cu_cp_paging_test, when_valid_paging_message_received_then_paging_is_sent_to_du)
{
  ...
  send_minimal_ngap_paging_and_await_f1ap_paging(du_idx);
}
...
TEST_F(cu_cp_paging_test, when_valid_paging_message_received_then_paging_is_only_sent_to_du_with_matching_tac)
{
  ...
  srsran_assert(not this->get_du(du_idx2).try_pop_dl_pdu(f1ap_pdu), "Paging was sent to DU with different TAC");
}
...
TEST_F(cu_cp_paging_test, when_valid_paging_message_with_optional_values_received_then_paging_is_sent_to_du)
{
  ...
  send_ngap_paging_and_await_f1ap_paging(du_idx);
}
