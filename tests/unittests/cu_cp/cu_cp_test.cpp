...
TEST_F(cu_cp_test, when_new_du_connection_then_du_added)
{
  ...
  cu_cp_obj->get_cu_cp_du_interface().handle_new_du_connection();

  // check that DU has been added
  ASSERT_EQ(cu_cp_obj->get_cu_cp_du_interface().get_nof_dus(), 1U);
}
...
TEST_F(cu_cp_test, when_du_remove_request_received_then_du_removed)
{
  ...
  // Remove DU
  cu_cp_obj->get_cu_cp_du_interface().handle_du_remove_request(uint_to_du_index(0));

  // Check that DU has been removed
  ASSERT_EQ(cu_cp_obj->get_cu_cp_du_interface().get_nof_dus(), 0U);
}
...
TEST_F(cu_cp_test, when_max_nof_dus_connected_then_reject_new_connection)
{
  ...
  ASSERT_EQ(cu_cp_obj->get_cu_cp_du_interface().get_nof_dus(), MAX_NOF_DUS);
...
TEST_F(cu_cp_test, when_amf_connected_then_ue_added)
{
  ...
  cu_cp_obj->get_cu_cp_du_interface().get_f1ap_message_handler(du_index).handle_message(f1setup_msg);
  ...
  ASSERT_EQ(cu_cp_obj->get_cu_cp_du_interface().get_nof_ues(), 1U);
...
TEST_F(cu_cp_test, when_amf_not_connected_then_ue_rejected)
{
  ...
  cu_cp_obj->get_cu_cp_du_interface().get_f1ap_message_handler(du_index).handle_message(init_ul_rrc_msg);
  ...
  cu_cp_obj->get_cu_cp_du_interface().get_f1ap_message_handler(du_index).handle_message(
      ue_context_release_complete_msg);

  // check that UE has not been added
  ASSERT_EQ(cu_cp_obj->get_cu_cp_du_interface().get_nof_ues(), 0U);
...
TEST_F(cu_cp_test, when_amf_connection_drop_then_reject_ue)
{
  ...
  ASSERT_EQ(cu_cp_obj->get_cu_cp_du_interface().get_f1ap_statistics_handler(du_index).get_nof_ues(), 1);
}
...
TEST_F(cu_cp_test, when_valid_paging_message_received_then_paging_is_sent_to_du)
{
  ...
  f1ap_message f1setup_msg = generate_f1_setup_request(0);
  // Pass message to CU-CP
  cu_cp_obj->get_cu_cp_du_interface().get_f1ap_message_handler(uint_to_du_index(0)).handle_message(f1setup_msg);
...
TEST_F(cu_cp_test, when_du_initiated_ue_context_release_received_then_ue_context_release_request_is_sent)
{
  ...
  cu_cp_obj->get_cu_cp_du_interface()
      .get_f1ap_message_handler(uint_to_du_index(0))
...
TEST_F(cu_cp_test, when_reestablishment_fails_then_ue_released)
{
  ...
  // Attach second UE with RRC Reestablishment Request
  {
    ...
    cu_cp_obj->get_cu_cp_du_interface().get_f1ap_message_handler(du_index).handle_message(ul_rrc_msg);
  ...
  ASSERT_EQ(cu_cp_obj->get_cu_cp_du_interface().get_nof_ues(), 2U);
}
