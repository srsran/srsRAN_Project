                                                      ...
                                                      std::initializer_list<drb_id_t> drbs_to_mod)
{
  ...
  for (drb_id_t drb_id : drbs_to_add) {
    ...
    req.drbs_to_setup.back().qos_info.drb_qos.qos_characteristics.non_dyn_5qi.emplace().five_qi = uint_to_five_qi(9);
...
du_manager_test_bench::du_manager_test_bench(span<const du_cell_config> cells) :
  ...
  params{{"srsgnb", (gnb_du_id_t)1, 1, transport_layer_address::create_from_string("127.0.0.1"), du_cells},
