...
static du_cell_config
make_custom_du_cell_config_for_format_0(const pucch_cnt_builder_params& pucch_params_,
                                        const srsran::config_helpers::cell_config_builder_params_extended& params = {})
{
  du_cell_config du_cfg          = config_helpers::make_default_du_cell_config(params);
  auto&          pucch_params    = du_cfg.pucch_cfg;
  ...
  auto& pucch_res_list = du_cfg.ue_ded_serv_cell_cfg.ul_config.value().init_ul_bwp.pucch_cfg.value().pucch_res_list;
  for (auto& res : pucch_res_list) {
    if (res.format == srsran::pucch_format::FORMAT_1) {
      res.format = srsran::pucch_format::FORMAT_0;
...
class du_ran_res_mng_pucch_format_0_tester : public du_ran_resource_manager_tester_base,
                                             public ::testing::TestWithParam<pucch_cnt_builder_params>
{
protected:
  explicit du_ran_res_mng_pucch_format_0_tester(cell_config_builder_params params_ = {.dl_arfcn = 520002U}) :
    du_ran_resource_manager_tester_base(params_,
                                        make_custom_du_cell_config_for_format_0(GetParam(), params_),
                                        GetParam().max_allowed_pucch_grants)
  {
    srsran_assert(default_ue_cell_cfg.csi_meas_cfg.has_value() and
                      not default_ue_cell_cfg.csi_meas_cfg.value().csi_report_cfg_list.empty() and
...
TEST_P(du_ran_res_mng_pucch_format_0_tester, sr_and_csi_pucch_resources_have_overlapping_symbols)
{
  du_ue_index_t next_ue_index = to_du_ue_index(0);
  ...
  for (unsigned i = 0; i != 2; ++i) {
    ...
    sr_offsets.insert(std::make_pair(sr_res_list[0].pucch_res_id.cell_res_id, sr_res_list[0].offset));
    unsigned    sr_offset        = sr_res_list[0].offset;
    const auto& sr_pucch_res_cfg = ue_res->cells[0]
                                       .serv_cell_cfg.ul_config->init_ul_bwp.pucch_cfg.value()
                                       .pucch_res_list[sr_res_list[0].pucch_res_id.ue_res_id];
    ASSERT_TRUE(sr_pucch_res_cfg.format == pucch_format::FORMAT_0);
    ASSERT_TRUE(std::holds_alternative<pucch_format_0_cfg>(sr_pucch_res_cfg.format_params));
    ...
    ASSERT_EQ(csi_offsets.count(std::make_pair(csi_pucch_res_id, csi_offset)), 0);
    csi_offsets.insert(std::make_pair(csi_pucch_res_id, csi_offset));
    const auto& csi_pucch_res_cfg = ue_res->cells[0]
                                        .serv_cell_cfg.ul_config->init_ul_bwp.pucch_cfg.value()
                                        .pucch_res_list[ue_csi_cfg.pucch_csi_res_list.front().pucch_res_id.ue_res_id];
    ASSERT_TRUE(csi_pucch_res_cfg.format == pucch_format::FORMAT_2);
    ASSERT_TRUE(std::holds_alternative<pucch_format_2_3_cfg>(csi_pucch_res_cfg.format_params));
    ...
    const auto&       sr_pucch_cfg  = std::get<pucch_format_0_cfg>(sr_pucch_res_cfg.format_params);
    const auto&       csi_pucch_cfg = std::get<pucch_format_2_3_cfg>(csi_pucch_res_cfg.format_params);
    ofdm_symbol_range sr_symbols{sr_pucch_cfg.starting_sym_idx,
                                 sr_pucch_cfg.starting_sym_idx + sr_pucch_cfg.nof_symbols};
    ofdm_symbol_range csi_symbols{csi_pucch_cfg.starting_sym_idx,
                                  csi_pucch_cfg.starting_sym_idx + csi_pucch_cfg.nof_symbols};
    ASSERT_TRUE(sr_symbols.overlaps(csi_symbols));
...
INSTANTIATE_TEST_SUITE_P(
    parametrized_test_for_format_0,
    du_ran_res_mng_pucch_format_0_tester,
    // clang-format off
       ::testing::Values(
       pucch_cnt_builder_params{ 7, 7, 15, srsran::sr_periodicity::sl_10, srsran::csi_report_periodicity::slots10 },
       pucch_cnt_builder_params{ 9, 9, 18, srsran::sr_periodicity::sl_20, srsran::csi_report_periodicity::slots20 },
       pucch_cnt_builder_params{ 11, 11, 22, srsran::sr_periodicity::sl_20, srsran::csi_report_periodicity::slots20 }
       )
    // clang-format on
);