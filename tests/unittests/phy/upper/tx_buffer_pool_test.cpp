...
TEST(tx_buffer_pool, codeblock_limit)
{
  ...
  unique_tx_buffer      buffer = pool->get_pool().reserve(slot, buffer_id0, pool_config.nof_codeblocks, true);
...
TEST(tx_buffer_pool, buffer_renew_expire)
{
  ...
  unique_tx_buffer locked_buffer = pool->get_pool().reserve(slot, buffer_id0, pool_config.nof_codeblocks, true);
  ...
  do {
    // Try to reserve another buffer. As there are no buffers available it shall fail.
    trx_buffer_identifier buffer_id1(0x1234, buffer_id0.get_harq() + 1);
    unique_tx_buffer      invalid_buffer = pool->get_pool().reserve(slot, buffer_id1, pool_config.nof_codeblocks, true);
...
TEST(tx_buffer_pool, buffer_resize)
{
  ...
  trx_buffer_identifier buffer_id0(0x1234, 0x3);
  unique_tx_buffer      buffer = pool->get_pool().reserve(slot, buffer_id0, nof_codeblocks, true);
  ASSERT_TRUE(buffer.is_valid());
  ...
  buffer = pool->get_pool().reserve(slot, buffer_id0, nof_codeblocks - 1, true);
...
TEST(tx_buffer_pool, buffer_resize_false_retransmission)
{
  static constexpr unsigned nof_codeblocks = 4;
...
TEST(tx_buffer_pool, fresh_false_retransmission)
{
  static constexpr unsigned nof_codeblocks = 4;
  ...
  trx_buffer_identifier buffer_id0(0x1234, 0x3);
  unique_tx_buffer      buffer = pool->get_pool().reserve(slot, buffer_id0, nof_codeblocks, false);
  ASSERT_FALSE(buffer.is_valid());
}

// Tests that the pool renews buffer expiration if they are locked.
TEST(tx_buffer_pool, reserve_no_id)
{
  static constexpr unsigned nof_codeblocks = 4;
  ...
  pool_config.max_codeblock_size   = 16;
  pool_config.nof_buffers          = 2;
  pool_config.nof_codeblocks       = pool_config.nof_buffers * nof_codeblocks;
  pool_config.expire_timeout_slots = 4;
  pool_config.external_soft_bits   = false;
  ...
  // Reserve more codeblocks than configured, it shall fail.
  ASSERT_FALSE(pool->get_pool().reserve(slot, pool_config.nof_codeblocks + 1).is_valid());

  // Reserve first buffer, it shall not fail.
  unique_tx_buffer buffer0 = pool->get_pool().reserve(slot, nof_codeblocks);
  ASSERT_TRUE(buffer0.is_valid());

  // Reserve second buffer, it shall not fail.
  unique_tx_buffer buffer1 = pool->get_pool().reserve(slot, nof_codeblocks);
  ASSERT_TRUE(buffer1.is_valid());

  // Reserve a third non-existent buffer, it shall fail.
  unique_tx_buffer buffer2 = pool->get_pool().reserve(slot, nof_codeblocks);
  ASSERT_FALSE(buffer2.is_valid());
}
...
TEST(tx_buffer_pool, buffer_contents)
{
  ...
  {
    // Reserve buffer, it shall not fail.
    unique_tx_buffer rm_buffer = pool->get_pool().reserve(slot, buffer_id, nof_cb_x_buffer, true);
  ...
  // Reserve buffer, it shall not fail.
  unique_tx_buffer buffer = pool->get_pool().reserve(slot, buffer_id, nof_cb_x_buffer, true);
  ASSERT_TRUE(buffer.is_valid());
...
TEST(tx_buffer_pool, reserve_after_stop)
{
  ...
  // Try to reserve with identifier, the buffer shall be invalid.
  ASSERT_FALSE(
      pool->get_pool().reserve(slot, trx_buffer_identifier(0, 0), pool_config.nof_codeblocks, true).is_valid());

  // Try to reserve without identifier, the buffer shall be invalid.
  ASSERT_FALSE(pool->get_pool().reserve(slot, pool_config.nof_codeblocks).is_valid());
}
