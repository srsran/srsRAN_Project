...
TEST_F(ResourceGridMapperFixture, SinglePort)
{
  ...
  assert_grid(golden, allocation, *grid, MAX_RB);
}
...
// make_wideband_diversity.
TEST_F(ResourceGridMapperFixture, TransmitDiversity)
{
  constexpr unsigned nof_layers = 1;
  ...
  for (unsigned nof_ports = 1; nof_ports <= precoding_constants::MAX_NOF_PORTS; ++nof_ports) {
    ...
    precoding_configuration precoding_config = make_wideband_diversity(nof_ports);
...
// Tests mapping for an identity matrix precoding configuration, generated by the function make_wideband_identity.
