...
f1ap_message generate_paging_message(uint64_t five_g_tmsi)
{
  f1ap_message msg;
  ...
  asn1_paging_cell_item.nr_cgi.nr_cell_id.from_number(config_helpers::make_nr_cell_identity(0, 32, 1));
  asn1_paging_cell_item.nr_cgi.plmn_id.from_string("00F110");
...
TEST_F(paging_tester, when_paging_message_is_received_its_relayed_to_ue)
{
  ...
  this->du_obj.get_f1ap_message_handler().handle_message(generate_paging_message(five_g_tmsi));
