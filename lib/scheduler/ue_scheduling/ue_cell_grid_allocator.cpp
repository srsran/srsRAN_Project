...
bool ue_cell_grid_allocator::allocate_dl_grant(const ue_pdsch_grant& grant)
{
  ...
  switch (dci_type) {
    ...
    case dci_dl_rnti_config_type::c_rnti_f1_1:
      pdsch_cfg =
          get_pdsch_config_f1_1_c_rnti(ue_cell_cfg, pdsch_list[grant.time_res_index], ue_cc->get_nof_dl_layers());
      break;
    default:
...
bool ue_cell_grid_allocator::allocate_ul_grant(const ue_pusch_grant& grant)
{
  ...
  switch (ss_info->get_crnti_ul_dci_format()) {
    ...
    case dci_ul_rnti_config_type::c_rnti_f0_1:
      pusch_cfg = get_pusch_config_f0_1_c_rnti(ue_cell_cfg, pusch_td_cfg, ue_cc->get_nof_ul_layers());
      break;
    default:
      report_fatal_error("Unsupported PDCCH DCI UL format");
  }
