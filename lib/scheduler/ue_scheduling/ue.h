...
public:
  ...
  /// \brief Defines a subPDU, containing LCID and payload size of UE Contention Resolution Identity CE if it is
  /// pending.
  /// \return Returns the number of bytes reserved in the TB for subPDU (other than padding).
  unsigned build_dl_con_res_ce_transport_block_info(dl_msg_tb_info& tb_info, unsigned tb_size_bytes);
