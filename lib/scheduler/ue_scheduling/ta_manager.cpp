...
ta_manager::ta_manager(subcarrier_spacing ul_scs_, dl_logical_channel_manager* dl_lc_ch_mgr_) :
  ul_scs(ul_scs_), dl_lc_ch_mgr(dl_lc_ch_mgr_), state(state_t::idle)
{
}

void ta_manager::handle_ul_n_ta_update_indication(uint8_t tag_id, int64_t n_ta_diff_)
{
  if (state == state_t::measure and not tag_n_ta_diff_measurements[tag_id].full()) {
...
void ta_manager::slot_indication(slot_point current_sl)
{
  ...
  if (abs(current_sl - meas_start_time) < MEAS_TIME_INTERVAL_IN_SLOTS) {
    return;
  }

  // Early return if change in Timing Advance Command is less than threshold.
  // NOTE: The new Timing Advance Command is a value ranging from [0,...,63] as per TS 38.213, clause 4.2. Hence, we
  ...
  for (uint8_t tag_id = 0; tag_id < tag_n_ta_diff_measurements.size(); ++tag_id) {
    ...
    if (abs((int)new_t_a - 31) < T_A_OFFSET_THRESHOLD) {
      continue;
    }
