  ...
  expert_cfg(expert_cfg_), cell_cfg(cell_cfg_), pdcch_sch(pdcch_sch_), logger(srslog::fetch_basic_logger("MAC"))
{
  ...
  if (cell_cfg.dl_cfg_common.init_dl_bwp.pdcch_common.paging_search_space_id.has_value() &&
    ...
    paging_period = std::max(ssb_periodicity_to_value(cell_cfg.ssb_cfg.ssb_period),
...
void ue_paging_scheduler::schedule_paging(cell_slot_resource_allocator& res_grid, slot_point sl_point)
{
  ...
  if (not cell_cfg.dl_cfg_common.init_dl_bwp.pdcch_common.paging_search_space_id.has_value()) {
    return;
  }
  ...
  for (const auto& pg_ind : paging_pending_ues) {
    ...
    for (unsigned time_res_idx = 0;
         ...
         ++time_res_idx) {
      if (schedule_paging_in_search_space0(
                                                           ...
                                                           unsigned                      paging_msg_size)
{
  ...
  const unsigned paging_period_slots = paging_period * static_cast<unsigned>(sl_point.nof_slots_per_subframe());
  ...
  for (unsigned ssb_idx = 0; ssb_idx < MAX_NUM_BEAMS; ssb_idx++) {
    ...
    if (sl_point.to_uint() % paging_period_slots == type0_pdcch_css_n0_slots[ssb_idx].to_uint()) {
      ...
      return allocate_paging_in_search_space0(res_grid, pdsch_time_res, ue_id, ssb_idx, paging_msg_size);
    }
  }
...
bool ue_paging_scheduler::allocate_paging_in_search_space0(cell_slot_resource_allocator& res_grid,
                                                           unsigned                      pdsch_time_res,
                                                           unsigned                      ue_id,
                                                           unsigned                      beam_idx,
                                                           unsigned                      paging_msg_size)
{
  ...
  crb_interval paging_crbs;
  {
    ...
    const prb_bitmap& used_crbs      = res_grid.dl_res_grid.used_crbs(
        coreset0_bwp_cfg, cell_cfg.dl_cfg_common.init_dl_bwp.pdsch_common.pdsch_td_alloc_list[pdsch_time_res].symbols);
  ...
  pdcch_dl_information* pdcch =
      pdcch_sch.alloc_pdcch_common(res_grid,
                                   rnti_t::P_RNTI,
                                   cell_cfg.dl_cfg_common.init_dl_bwp.pdcch_common.sib1_search_space_id,
                                   expert_cfg.pg.paging_dci_aggr_lev);
  if (pdcch == nullptr) {
                                            ...
                                            unsigned                      tbs)
{
  ...
  prb_interval paging_prbs  = crb_to_prb(coreset0_bwp_cfg, crbs_grant);
