...
static bool is_csi_slot_offset_valid(unsigned                       slot_offset,
                                     const tdd_ul_dl_config_common& tdd_cfg,
                                     span<const unsigned>           tracking_csi_ofdm_symbol_indexes,
                                     unsigned                       ssb_period_ms)
{
  ...
  srsran_assert(not tracking_csi_ofdm_symbol_indexes.empty(), "Invalid tracking CSI-RS symbol indexes");
  const unsigned max_csi_symbol =
      *std::max_element(tracking_csi_ofdm_symbol_indexes.begin(), tracking_csi_ofdm_symbol_indexes.end());
  return get_active_tdd_dl_symbols(tdd_cfg, slot_index, cyclic_prefix::NORMAL).length() > max_csi_symbol;
}
                                                          ...
                                                          unsigned             ssb_period_ms)
{
  ...
  if (meas_csi_slot_offset.has_value()) {
    if (not is_csi_slot_offset_valid(*meas_csi_slot_offset, tdd_cfg, tracking_csi_ofdm_symbol_indexes, ssb_period_ms)) {
      return false;
    }
  ...
  if (tracking_csi_slot_offset.has_value()) {
    ...
    if (not is_csi_slot_offset_valid(
            *tracking_csi_slot_offset, tdd_cfg, tracking_csi_ofdm_symbol_indexes, ssb_period_ms) or
        not is_csi_slot_offset_valid(
            *tracking_csi_slot_offset + 1, tdd_cfg, tracking_csi_ofdm_symbol_indexes, ssb_period_ms)) {
      return false;
    }
    csi_params.tracking_csi_slot_offset = *tracking_csi_slot_offset;
  }
  if (zp_csi_slot_offset.has_value()) {
    if (not is_csi_slot_offset_valid(*zp_csi_slot_offset, tdd_cfg, tracking_csi_ofdm_symbol_indexes, ssb_period_ms)) {
      return false;
    }
  ...
  for (unsigned i = 0; i < static_cast<unsigned>(csi_params.csi_rs_period) and (not meas_found or not tracking_found);
       ++i) {
    if (not is_csi_slot_offset_valid(i, tdd_cfg, tracking_csi_ofdm_symbol_indexes, ssb_period_ms)) {
      continue;
    }
    // Note: Tracking CSI-RS occupies two consecutive slots.
    if (not tracking_found and
        is_csi_slot_offset_valid(i + 1, tdd_cfg, tracking_csi_ofdm_symbol_indexes, ssb_period_ms) and
