                                                                        ...
                                                                        const pdcch_dl_information& dci_info)
{
  ...
  cell_slot_resource_allocator& pucch_slot_alloc = slot_alloc[k0 + k1 + ntn_cs_koffset];

  if (pucch_slot_alloc.result.ul.pucchs.full() or
      pucch_common_alloc_grid[slot_alloc[k0 + k1 + ntn_cs_koffset].slot.to_uint()].full()) {
    return nullopt;
  }
  ...
  pucch_common_alloc_grid[slot_alloc[k0 + k1 + ntn_cs_koffset].slot.to_uint()].emplace_back(tcrnti);

  {
    fmt::memory_buffer fmtbuf;
    for (auto pucch_com_rnti : pucch_common_alloc_grid[slot_alloc[k0 + k1 + ntn_cs_koffset].slot.to_uint()]) {
    ...
    logger.debug("List of common PUCCH res scheduled currently allocated in this slot. List size={} Content: {}",
                 pucch_common_alloc_grid[slot_alloc[k0 + k1 + ntn_cs_koffset].slot.to_uint()].size(),
                 to_c_str(fmtbuf));
  }
                                                                     ...
                                                                     unsigned                     ntn_cs_koffset)
{
  ...
  // Get the slot allocation grid considering the PDSCH delay (k0) and the PUCCH delay wrt PDSCH (k1).
  cell_slot_resource_allocator& pucch_slot_alloc = res_alloc[k0 + k1 + ntn_cs_koffset];
