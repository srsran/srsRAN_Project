...
std::unique_ptr<pdcp_input_gateway> srsgnb::create_pdcp(pdcp_packet_notifier& listener)
{
  auto ul_procedure =
      std::unique_ptr<pdcp_packet_procedures>(new pdcp_ul_packet_procedure(create_pdcp_entity(), listener));

  return std::unique_ptr<pdcp_input_gateway>(new pdcp_procedure_dispatcher(std::move(ul_procedure)));
}
