...
{
  logger_cu.info("Creating CU F1-U bearer. UL-TEID={}", ul_up_tnl_info.gtp_teid);
  std::unique_lock<std::mutex> lock(map_mutex);
  srsran_assert(cu_map.find(ul_up_tnl_info) == cu_map.end(),
                "Cannot create CU F1-U bearer with already existing UL-TEID={}",
                ul_up_tnl_info.gtp_teid);
                                         ...
                                         const up_transport_layer_info& dl_up_tnl_info)
{
  ...
  if (cu_map.find(ul_up_tnl_info) == cu_map.end()) {
    logger_cu.warning("Could not find UL-TEID at CU to connect. UL-TEID={}, DL-TEID={}",
                      ul_up_tnl_info.gtp_teid,
                      dl_up_tnl_info.gtp_teid);
    return;
  }
  logger_cu.debug(
      "Connecting CU F1-U bearer. UL-TEID={}, DL-TEID={}", ul_up_tnl_info.gtp_teid, dl_up_tnl_info.gtp_teid);

  if (du_map.find(dl_up_tnl_info) == du_map.end()) {
    logger_cu.warning("Could not find DL-TEID at DU to connect. UL-TEID={}, DL-TEID={}",
                      ul_up_tnl_info.gtp_teid,
                      dl_up_tnl_info.gtp_teid);
    return;
  }
  logger_cu.debug(
      "Connecting DU F1-U bearer. UL-TEID={}, DL-TEID={}", ul_up_tnl_info.gtp_teid, dl_up_tnl_info.gtp_teid);
...
void f1u_local_connector::disconnect_cu_bearer(const up_transport_layer_info& ul_up_tnl_info)
{
  ...
  if (bearer_it == cu_map.end()) {
    logger_cu.warning("Could not find UL-TEID={} at CU to remove.", ul_up_tnl_info.gtp_teid);
    return;
  }
  ...
  if (bearer_it->second.dl_up_tnl_info.has_value()) {
    ...
    if (du_bearer_it != du_map.end()) {
      logger_cu.debug("Disconnecting DU F1-U bearer with DL-TEID={} from CU handler. UL-TEID={}",
                      bearer_it->second.dl_up_tnl_info->gtp_teid,
                      ul_up_tnl_info.gtp_teid);
    ...
    logger_cu.warning("No DL-TEID provided to disconnect DU F1-U bearer from CU handler. UL-TEID={}",
                      ul_up_tnl_info.gtp_teid);
  }

  // Remove DL path
  logger_cu.debug("Removing CU F1-U bearer with UL-TEID={}.", ul_up_tnl_info.gtp_teid);
                                                          ...
                                                          timer_factory                  timers)
{
  std::unique_lock<std::mutex> lock(map_mutex);
  if (cu_map.find(ul_up_tnl_info) == cu_map.end()) {
    logger_du.warning("Could not find CU F1-U bearer, when creating DU F1-U bearer. DL-TEID={}, UL-TEID={}",
                      dl_up_tnl_info.gtp_teid,
                      ul_up_tnl_info.gtp_teid);
    return nullptr;
  }

  logger_du.debug("Creating DU F1-U bearer. DL-TEID={}, UL-TEID={}", dl_up_tnl_info.gtp_teid, ul_up_tnl_info.gtp_teid);
...
void f1u_local_connector::remove_du_bearer(const up_transport_layer_info& dl_up_tnl_info)
{
  ...
  if (du_bearer_it == du_map.end()) {
    logger_du.warning("Could not find DL-TEID at DU to remove. DL-TEID={}", dl_up_tnl_info.gtp_teid);
    return;
  }
  logger_du.debug("Removing DU F1-U bearer. DL-TEID={}", dl_up_tnl_info.gtp_teid);

  auto cu_bearer_it = cu_map.find(du_bearer_it->second.ul_up_tnl_info);
  if (cu_bearer_it != cu_map.end()) {
    logger_du.debug("Detaching CU handler do to removal at DU. UL-TEID={}",
                    du_bearer_it->second.ul_up_tnl_info.gtp_teid);
