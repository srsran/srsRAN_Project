...
{
public:
  ue_bearer_manager(f1c_ue_context& ue_ctx_, f1c_message_notifier& f1c_notif_) :
    ue_ctx(ue_ctx_), f1c_notifier(f1c_notif_)
  {
  }

  void add_srb0(f1_tx_pdu_notifier&        f1_tx_pdu_notif,
                const asn1::f1ap::nrcgi_s& pcell_cgi,
                const byte_buffer&         du_cu_rrc_container,
                f1ap_event_manager&        ev_mng);

  void add_srb(srb_id_t srb_id, f1_tx_pdu_notifier& f1_tx_pdu_notif);

  void add_drb(drb_id_t drb_id, f1u_tx_pdu_notifier& f1_tx_pdu_notif);
