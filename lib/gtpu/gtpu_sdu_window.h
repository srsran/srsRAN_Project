...
template <class T>
class gtpu_sdu_window_base
{
public:
  virtual ~gtpu_sdu_window_base()          = default;
...
/// \brief This class provides a container for the Rx windows holding GTP-U SDUs that are indexed by
/// Sequence Numbers (SN)
/// @tparam T storage type
/// @tparam WINDOW_SIZE size of the window
template <class T, std::size_t WINDOW_SIZE>
class gtpu_sdu_window final : public gtpu_sdu_window_base<T>
{
public:
  gtpu_sdu_window(gtpu_tunnel_logger& logger_) : logger(logger_) {}
  ~gtpu_sdu_window() = default;
