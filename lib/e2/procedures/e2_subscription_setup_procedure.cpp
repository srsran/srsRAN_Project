...
void e2_subscription_setup_procedure::send_e2_subscription_setup_response(const e2_subscribe_reponse_message &response)
{
  ...
  for (int i = 0; i < size; i++) {
    sub_resp->ri_cactions_admitted.value[0].value().ri_caction_admitted_item().ric_action_id = response.admitted_list[i].value().ri_caction_admitted_item().ric_action_id;
  }
