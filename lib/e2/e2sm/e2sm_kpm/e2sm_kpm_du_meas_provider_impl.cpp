                                                  ...
                                                  std::vector<asn1::e2sm::meas_record_item_c>& items)
{
  ...
  if (ues.empty()) {
    double mean_ul_delay_ms =
        std::accumulate(last_ue_metrics.begin(),
                        last_ue_metrics.end(),
                        0,
                        [](size_t sum, const scheduler_ue_metrics& metric) { return sum + metric.avg_crc_delay_ms; }) /
  ...
  for (auto& ue : ues) {
    ...
    uint32_t            ue_idx            = f1ap_ue_id_provider.get_ue_index(gnb_cu_ue_f1ap_id);
    meas_record_item_c  meas_record_item;
    if (last_ue_metrics[ue_idx].avg_crc_delay_ms) {
      meas_record_item.set_real().value = static_cast<float>(last_ue_metrics[ue_idx].avg_crc_delay_ms);
    } else {
      meas_record_item.set_no_value();
    }
    items.push_back(meas_record_item);
