...
{
  return std::make_unique<phy_fapi_adaptor_impl>(config.sector_id, dl_processor_pool, rg_pool, config.scs_common);
}
...
{
  return std::make_unique<phy_fapi_adaptor_factory_impl>(dl_processor_pool, rg_pool);
}
