...
fapi_to_mac_time_msg_fastpath_translator::fapi_to_mac_time_msg_fastpath_translator(subcarrier_spacing scs_) :
  scs(scs_), mac_slot_handler(&mac_dummy_handler)
{
}

void fapi_to_mac_time_msg_fastpath_translator::on_slot_indication(const fapi::slot_indication_message& msg)
{
  mac_slot_handler->handle_slot_indication({slot_point(to_numerology_value(scs), msg.sfn, msg.slot), msg.time_point});
}
