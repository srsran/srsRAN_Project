...
void ldpc_decoder_avx2::load_soft_bits(span<const log_likelihood_ratio> llrs)
{
  ...
  unsigned node_size = node_size_avx2 * AVX2_SIZE_BYTE;
  ...
  for (unsigned i_node = 2 * node_size_avx2, max_node = nof_full_nodes * node_size_avx2; i_node != max_node;
  ...
  if (!llrs.empty()) {
    ...
    span<log_likelihood_ratio> remaining_node(
        reinterpret_cast<log_likelihood_ratio*>(soft_bits.data_at(nof_full_nodes * node_size_avx2)), node_size);
...
void ldpc_decoder_avx2::get_hard_bits(bit_buffer& out)
{
  ...
  // efficiency.
  for (unsigned i_node_lifted = 0, max_node_lifted = bg_K * node_size_avx2; i_node_lifted != max_node_lifted;
       i_node_lifted += node_size_avx2) {
    // View over the LLR.
                                   ...
                                   const mm256::avx2_span& this_var_to_check)
{
  for (unsigned i_block = 0; i_block != NODE_SIZE_AVX2; ++i_block) {
    ...
    __m256i check_to_var_epi8         = this_check_to_var.get_at(i_block);
    ...
    __m256i var_to_check_epi8       = this_var_to_check.get_at(i_block);
    ...
    __m256i soft_epi8 = _mm256_adds_epi8(check_to_var_epi8, this_var_to_check.get_at(i_block));
