...
void ldpc_decoder_avx2::update_variable_to_check_messages(unsigned check_node)
{
  ...
  compute_var_to_check_msgs_avx2(mm256::avx2_span(var_to_check, 0, bg_N_hr_lifted),
  ...
  if (check_node >= 4) {
    compute_var_to_check_msgs_avx2(
...
void ldpc_decoder_avx2::compute_var_to_check_msgs_avx2(mm256::avx2_span       v2c,
                                                       mm256::avx2_const_span soft,
                                                       mm256::avx2_const_span c2v)
{
  unsigned length = v2c.size();
  srsgnb_assert((soft.size() == length) && (c2v.size() == length),
  ...
  for (this_var_index_itr = current_var_indices.cbegin();
    ...
    for (unsigned i_block = 0; i_block != node_size_avx2; ++i_block) {
      ...
      __m256i check_to_var_epi8 = _mm256_blendv_epi8(
          min_var_to_check.get_at(i_block), second_min_var_to_check.get_at(i_block), mask_is_min_epi8);
...
void ldpc_decoder_avx2::update_soft_bits(unsigned check_node)
{
  ...
  for (; (this_var_index != last_var_index) && (*this_var_index != NO_EDGE); ++this_var_index) {
    ...
    // For all AVX2 registers in the node...
    for (unsigned i_block = 0; i_block != node_size_avx2; ++i_block) {
