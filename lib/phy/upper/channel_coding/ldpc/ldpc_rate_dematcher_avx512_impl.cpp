                                                       ...
                                                       span<const log_likelihood_ratio> in1) const
{
  ...
  for (unsigned index_end = (out.size() / 64) * 64; index != index_end; index += 64) {
    __m512i in0_avx = _mm512_loadu_si512(reinterpret_cast<const __m512i*>(&in0[index]));
    __m512i in1_avx = _mm512_loadu_si512(reinterpret_cast<const __m512i*>(&in1[index]));

    __m512i result = _mm512_adds_epi8(in0_avx, in1_avx);
