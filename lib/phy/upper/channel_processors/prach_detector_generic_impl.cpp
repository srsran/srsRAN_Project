...
bool prach_detector_validator_impl::is_valid(const prach_detector::configuration& config)
{
  ...
  if ((config.format == prach_format_type::A1) || (config.format == prach_format_type::B4)) {
    ...
    if (config.ra_scs == prach_subcarrier_spacing::kHz15) {
      ...
      if ((config.zero_correlation_zone != 0) && (config.zero_correlation_zone != 14)) {
        return false;
      }
    } else {
      return false;
    }
  } else if (is_short_preamble(config.format)) {
    // Other short preambles are not supported.
    return false;
  }
...
prach_detection_result prach_detector_generic_impl::detect(const prach_buffer& input, const configuration& config)
{
  ...
  // Gets the number of preambles using the same root sequence (i.e., the number of cyclic shifts) and the total number
  // of root sequences needed.
  ...
  for (unsigned i_sequence = 0; i_sequence != nof_sequences; ++i_sequence) {
    ...
    for (unsigned i_port = 0; i_port != config.nof_rx_ports; ++i_port) {
      for (unsigned i_symbol = 0; i_symbol != 1; ++i_symbol) {
        ...
        srsvec::prod_conj(root, preamble, no_root);

        // Prepare IDFT for correlation.
        srsvec::copy(idft_input.first(L_ra / 2), no_root.last(L_ra / 2));
        srsvec::copy(idft_input.last(L_ra / 2 + 1), no_root.first(L_ra / 2 + 1));
        ...
        for (unsigned i_window = 0; i_window != nof_shifts; ++i_window) {
          ...
          unsigned window_start = (N_cs * i_window * DFT_SIZE) / L_ra;
