                                ...
                                const segmenter_config& cfg)
{
  ...
  span<uint8_t> tmp = span<uint8_t>(buffer_cb).first(d_segments[0].get_metadata().cb_specific.full_length);

  unsigned offset = 0;
  for (const auto& descr_seg : d_segments) {
    // Encode the segment into a codeblock.
    encoder->encode(tmp, descr_seg.get_data(), descr_seg.get_metadata().tb_common);
    ...
    rate_matcher->rate_match(codeblock, tmp, descr_seg.get_metadata().tb_common);
  }
}
