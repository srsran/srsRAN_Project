...
{
public:
  pusch_uci_decoder_wrapper(uci_decoder& decoder_, pusch_uci_decoder_notifier& notifier_, unsigned max_nof_softbits) :
    decoder(decoder_), notifier(notifier_), softbits_buffer(max_nof_softbits)
  {
  }

  pusch_decoder_buffer& new_transmission(unsigned message_length, modulation_scheme modulation)
  {
    softbits_count            = 0;
...
private:
  ...
  uci_decoder& decoder;
  /// Decode Notifier.
  pusch_uci_decoder_notifier& notifier;
