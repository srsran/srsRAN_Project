...
class dmrs_pucch_estimator_format1
{
public:
  using layer_dmrs_pattern = port_channel_estimator::layer_dmrs_pattern;

  dmrs_pucch_estimator_format1(std::unique_ptr<pseudo_random_generator>            prg_,
  ...
  /// Estimates the channel of a PUCCH Format 1 transmission. See \ref dmrs_pucch_estimator for more details.
...
private:
  ...
  static layer_dmrs_pattern generate_dmrs_pattern(const dmrs_pucch_estimator::format1_configuration& config);
};
