                                                          ...
                                                          const du_test_mode_config::test_mode_ue_config& test_ue_cfg)
{
  ...
  srsran_assert(pucch.get_format() == pucch_format::FORMAT_0 or pucch.get_format() == pucch_format::FORMAT_1,
  ...
  if (pucch.get_format() == pucch_format::FORMAT_0) {
    ...
    if (pucch.get_sr_bits() != sr_nof_bits::no_sr) {
    ...
    if (pucch.get_harq_ack_nof_bits() > 0) {
      ...
      pucch_ind.harq_info->harqs.resize(pucch.get_harq_ack_nof_bits(), uci_pucch_f0_or_f1_harq_values::ack);
    }
  } else {
    if (pucch.get_sr_bits() != sr_nof_bits::no_sr) {
      // In test mode, SRs are never detected, and instead BSR is injected.
    ...
    if (pucch.get_harq_ack_nof_bits() > 0) {
      ...
      auto ack_val = pucch.get_sr_bits() == sr_nof_bits::no_sr ? uci_pucch_f0_or_f1_harq_values::ack
                                                               : uci_pucch_f0_or_f1_harq_values::dtx;
      pucch_ind.harq_info->harqs.resize(pucch.get_harq_ack_nof_bits(), ack_val);
    }
  }
  return pucch_ind;
}
...
{
  srsran_assert(pucch.get_format() == pucch_format::FORMAT_2 or pucch.get_format() == pucch_format::FORMAT_3 or
                    pucch.get_format() == pucch_format::FORMAT_4,
...
bool srs_du::pucch_info_and_uci_ind_match(const pucch_info& pucch, const mac_uci_pdu& uci_ind)
{
  ...
  if ((pucch.get_format() == pucch_format::FORMAT_0 or pucch.get_format() == pucch_format::FORMAT_1) and
      std::holds_alternative<mac_uci_pdu::pucch_f0_or_f1_type>(uci_ind.pdu)) {
    const auto  pucch_pdu_sr_bits = pucch.get_sr_bits();
    ...
    const auto pucch_pdu_harq_bits = pucch.get_harq_ack_nof_bits();
    if (f0f1_ind.harq_info.has_value() != (pucch_pdu_harq_bits > 0)) {
      return false;
    }
    return true;
  }
  if ((pucch.get_format() == pucch_format::FORMAT_2 or pucch.get_format() == pucch_format::FORMAT_3 or
       pucch.get_format() == pucch_format::FORMAT_4) and
