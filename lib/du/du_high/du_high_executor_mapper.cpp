                                        ...
                                        executor_metrics_backend*                                  metrics_backend)
  {
    ...
    for (const auto& cell_worker : cell_workers) {
      std::string slot_exec_name =
          trace_enabled or metrics_backend ? fmt::format("slot_ind_exec#{}", cell_execs.size()) : "";
      std::string cell_exec_name =
          trace_enabled or metrics_backend ? fmt::format("cell_exec#{}", cell_execs.size()) : "";
      cell_execs.push_back(du_high_executor_config::dedicated_cell_worker{
          &decorator.decorate(
              *cell_worker.high_prio_executor, is_sync, trace_enabled, std::nullopt, metrics_backend, slot_exec_name),
          &decorator.decorate(
              *cell_worker.low_prio_executor, is_sync, trace_enabled, std::nullopt, metrics_backend, cell_exec_name)});
    }
  }
...
public:
                                     ...
                                     executor_metrics_backend*                                metrics_backend)
  {
    ...
    for (unsigned i = 0, e = cfg.nof_cells; i != e; ++i) {
      ...
      std::string exec_name = trace_enabled or metrics_backend ? fmt::format("slot_ind_exec#{}", i) : "";
      cell_strands[i].slot_ind_exec =
          &decorator.decorate(execs[0], is_sync, trace_enabled, std::nullopt, metrics_backend, exec_name);
      exec_name = trace_enabled or metrics_backend ? fmt::format("mac_cell_exec#{}", i) : "";
      cell_strands[i].mac_cell_exec =
          &decorator.decorate(execs[1], is_sync, trace_enabled, std::nullopt, metrics_backend, exec_name);
      exec_name = trace_enabled or metrics_backend ? fmt::format("rlc_lower_exec#{}", i) : "";
...
create_du_high_cell_executor_mapper(const du_high_executor_config& config)
{
  ...
  if (const auto* ded_workers =
    ...
    cell_mapper = std::make_unique<dedicated_cell_worker_executor_mapper>(
        *ded_workers, config.is_rt_mode_enabled, config.trace_exec_tasks, config.exec_metrics_backend);
  } else {
...
protected:
                  ...
                  executor_metrics_backend* metrics_backend)
  {
    ...
    if (trace_enabled or metrics_backend) {
      ...
      strand_ctxt.ul_exec   = &decorator.decorate(
          *strand_ctxt.ul_exec, false, trace_enabled, std::nullopt, metrics_backend, fmt::format("ue_ul_exec#{}", idx));
      strand_ctxt.dl_exec = &decorator.decorate(
          *strand_ctxt.dl_exec, false, trace_enabled, std::nullopt, metrics_backend, fmt::format("ue_dl_exec#{}", idx));
    }
  }
...
public:
    ...
    common_ue_executor_mapper(max_strands)
  {
    for (unsigned i = 0; i != max_strands; ++i) {
      add_strand(pool_executor, ctrl_queue_size, pdu_queue_size, trace_enabled, metrics_backend);
    }
  }
...
public:
  explicit pcell_ue_executor_mapper(task_executor&            pool_executor,
                                    ...
                                    bool                      trace_enabled_,
                                    executor_metrics_backend* metrics_backend_) :
    ...
    metrics_backend(metrics_backend_)
  {
    // Create base strand.
    add_strand(pool_executor, ctrl_queue_size, pdu_queue_size, trace_enabled, metrics_backend);
  ...
  void rebind_executors(du_ue_index_t ue_index, du_cell_index_t pcell_index) override
  {
    ...
    while (strands.size() <= ue_idx_to_exec_index[ue_index]) {
      add_strand(pool_exec, ctrl_queue_size, pdu_queue_size, trace_enabled, metrics_backend);
    }
  }
...
public:
  ctrl_executor_mapper(const du_high_executor_config::control_executor_config& cfg,
    ...
    ctrl_exec(decorator.decorate(strand,
                                 ...
                                 std::nullopt,
                                 metrics_backend,
                                 trace_enabled or metrics_backend ? "du_ctrl_exec" : "")),
    e2_exec(decorator.decorate(strand,
                               false,
                               trace_enabled,
                               std::nullopt,
                               metrics_backend,
                               trace_enabled or metrics_backend ? "du_e2_exec" : ""))
  {
  }
...
public:
  explicit du_high_executor_mapper_impl(const du_high_executor_config& config) :
    ...
    ctrl_mapper(config.ctrl_executors, config.trace_exec_tasks, config.exec_metrics_backend)
  {
  }
