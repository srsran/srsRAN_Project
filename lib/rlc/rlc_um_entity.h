...
public:
  ...
  {
    tx = std::unique_ptr<rlc_tx_entity>(new rlc_tx_um_entity(du_index, lcid, upper_cn));
    rx = std::unique_ptr<rlc_rx_entity>(new rlc_rx_um_entity(du_index, lcid, upper_dn));
  }
};
