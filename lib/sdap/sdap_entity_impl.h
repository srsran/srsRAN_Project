...
{
public:
  sdap_entity_impl(uint32_t ue_index, sdap_tx_pdu_notifier& tx_pdu_notifier, sdap_rx_sdu_notifier& rx_sdu_notifier) :
    logger(srslog::fetch_basic_logger("SDAP"))
  {
    rx = std::make_unique<sdap_entity_rx_impl>(ue_index, rx_sdu_notifier);
    tx = std::make_unique<sdap_entity_tx_impl>(ue_index, tx_pdu_notifier);
  }
