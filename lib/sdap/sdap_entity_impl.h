...
public:
  ...
  {
    rx = std::make_unique<sdap_entity_rx_impl>(ue_index, pdu_session_id, ue_inactivity_timer, rx_sdu_notifier);
    tx = std::make_unique<sdap_entity_tx_impl>(ue_index, pdu_session_id, ue_inactivity_timer, tx_pdu_notifier);
  }
