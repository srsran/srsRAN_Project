...
{
public:
  sdap_entity_impl(sdap_tx_pdu_notifier& tx_pdu_notifier, sdap_rx_sdu_notifier& rx_sdu_notifier) :
    logger(srslog::fetch_basic_logger("SDAP"))
  {
    rx = std::make_unique<sdap_entity_rx_impl>(rx_sdu_notifier);
    tx = std::make_unique<sdap_entity_tx_impl>(tx_pdu_notifier);
  }
