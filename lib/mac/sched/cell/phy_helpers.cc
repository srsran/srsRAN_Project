...
/// Convert SLIV to symbol start S and length L.
...
static void srsran_sliv_to_s_and_l(uint32_t N, uint32_t sliv, uint32_t& S, uint32_t& L)
{
  uint32_t low  = sliv % N;
                   ...
                   symbol_interval&                         symbols)
{
  ...
  srsran_assert(pusch_scs >= asn1::rrc_nr::subcarrier_spacing_opts::options::khz15 and
                    pusch_scs <= asn1::rrc_nr::subcarrier_spacing_opts::options::khz120,
                "Invalid PUSCH subcarrier spacing");

  /// The array represents Table 6.1.2.1.1-5, in TS 38.214.
  // TODO: Check if this is implemented in the PHY.
  const std::array<uint8_t, 4> DELTAS{2, 3, 4, 6};
  ...
  pusch_delay = static_cast<int>(k2 + DELTAS[pusch_scs.to_number()]);
  ...
  srsran_sliv_to_s_and_l(SRSRAN_NSYMB_PER_SLOT_NR, sliv, symbol_S, symbol_L);
