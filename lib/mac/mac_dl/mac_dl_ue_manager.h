...
private:
  ...
  bool addmod_bearers_nolock(du_ue_index_t ue_index, span<const logical_channel_addmod> dl_logical_channels)
  {
    if (not ue_db.contains(ue_index)) {
      return false;
    }
  ...
  mutable circular_array<std::mutex, MAX_NOF_UES> ue_mutex;
