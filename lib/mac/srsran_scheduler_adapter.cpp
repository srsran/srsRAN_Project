...
{
  // Conver MAC CRC indication to srsRAN scheduler UL CRC indication.
  ...
  for (unsigned i = 0; i != ind.crcs.size(); ++i) {
    ...
    if (pdu.ue_index != INVALID_DU_UE_INDEX) {
      rlf_handler.handle_crc(pdu.ue_index, pdu.tb_crc_success);
    }
  }
...
void srsran_scheduler_adapter::handle_uci(du_cell_index_t cell_idx, const mac_uci_indication_message& msg)
{
