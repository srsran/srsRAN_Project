...
void ue_configuration_procedure::add_bearers_to_ue_context()
{
  ...
  for (srb_id_t srbid : request.srbs_to_setup) {
    srsgnb_assert(not ue->bearers.srbs.contains(srbid), "Reconfigurations of bearers not supported");
    ue->bearers.srbs.emplace(srbid);
    du_ue_srb& srb = ue->bearers.srbs[srbid];
    srb.srb_id     = srbid;
    srb.rlc_cfg    = make_default_srb_rlc_config(); // TODO: Parameterize SRB config.
  }
  ...
  for (const drb_to_setup& drbtoadd : request.drbs_to_setup) {
    ...
    du_ue_drb& drb = ue->bearers.drbs[drbtoadd.drb_id];
    drb.drb_id     = drbtoadd.drb_id;
    drb.lcid       = lcid;
