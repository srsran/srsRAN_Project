...
du_ue_manager::du_ue_manager(du_manager_config_t& cfg_) : cfg(cfg_), logger(cfg.logger)
{
  ...
  for (size_t i = 0; i < MAX_DU_NOF_UES; ++i) {
...
{
  srsran_assert(ue_index < MAX_DU_NOF_UES, "Invalid ue_index={}", ue_index);
  return ue_db.contains(ue_index) ? &ue_db[ue_index] : nullptr;
}

du_ue_context* du_ue_manager::find_rnti(rnti_t rnti)
{
  if (rnti_to_ue_index[rnti % MAX_DU_NOF_UES] < 0) {
    return nullptr;
  }
  return &ue_db[rnti_to_ue_index[rnti % MAX_DU_NOF_UES]];
}

du_ue_context* du_ue_manager::add_ue(du_ue_context ue_ctx)
{
  srsran_assert(ue_ctx.rnti != INVALID_RNTI, "Invalid RNTI");
  srsran_assert(ue_ctx.ue_index < MAX_DU_NOF_UES, "Invalid ue_index={}", ue_ctx.ue_index);

  if (ue_db.contains(ue_ctx.ue_index) or rnti_to_ue_index[ue_ctx.rnti % MAX_DU_NOF_UES] >= 0) {
  ...
  srsran_sanity_check(rnti_to_ue_index[u.rnti % MAX_DU_NOF_UES] < 0, "Invalid RNTI=0x{:x}", u.rnti);
  rnti_to_ue_index[u.rnti % MAX_DU_NOF_UES] = ue_index;
...
void du_ue_manager::remove_ue(du_ue_index_t ue_index)
{
  ...
  srsran_assert(ue_index < MAX_DU_NOF_UES, "Invalid ueId={}", ue_index);
  ...
  ue_ctrl_loop[ue_index].schedule([this, ue_index](coro_context<async_task<void> >& ctx) {
    ...
    rnti_to_ue_index[ue_db[ue_index].rnti % MAX_DU_NOF_UES] = -1;
