...
du_ue_context* du_ue_manager::add_ue(du_ue_context ue_ctx)
{
  ...
  if (ue_db.contains(ue_ctx.ue_index) or rnti_to_ue_index[ue_ctx.rnti % MAX_NOF_UES] >= 0) {
  ...
  srsran_sanity_check(rnti_to_ue_index[u.rnti % MAX_NOF_UES] < 0, "Invalid RNTI=0x{:x}", u.rnti);
  rnti_to_ue_index[u.rnti % MAX_NOF_UES] = ue_index;
