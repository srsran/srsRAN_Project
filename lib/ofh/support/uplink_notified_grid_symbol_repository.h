...
class uplink_notified_grid_symbol_repository
{
  ...
  static constexpr uint16_t USED_SLOT_BITMAP                 = 1u << USED_SLOT_BIT;
  static constexpr uint16_t EXTENDED_CP_BITMAP               = 1u << EXTENDED_CP_BIT;
  static constexpr uint16_t ALL_SYMBOLS_NOTIFIED_CP_NORMAL   = (1u << get_nsymb_per_slot(cyclic_prefix::NORMAL)) - 1;
  static constexpr uint16_t ALL_SYMBOLS_NOTIFIED_CP_EXTENDED = (1u << get_nsymb_per_slot(cyclic_prefix::EXTENDED)) - 1;

  using repo_entry = std::atomic<uint16_t>;
  std::vector<repo_entry> repo;

  /// Returns the entry for the given slot.
  std::atomic<uint16_t>& get_entry(slot_point slot) { return repo[slot.system_slot() % repo.size()]; }

  /// Returns true if the slot is used, otherwise false.
  static constexpr bool is_slot_used(uint16_t value) { return value & USED_SLOT_BITMAP; }

  /// Returns true if all symbols have been notified, otherwise false.
  static constexpr bool are_all_symbols_notified(uint16_t value)
  {
  ...
  static constexpr bool is_any_previous_symbol_missed(uint16_t value, unsigned symbol)
  {
  ...
  static constexpr unsigned get_nof_symbols(uint16_t value)
  {
  ...
  static constexpr unsigned get_nof_symbols_to_notify(uint16_t value, unsigned symbol)
  {
...
public:
  ...
  std::optional<interval<unsigned>> update_rx_symbol_and_compute_symbols_to_notify(slot_point slot, unsigned symbol)
  {
    srsran_assert(symbol < MAX_NSYMB_PER_SLOT, "Invalid symbol '{}'", symbol);

    uint16_t value = get_entry(slot).fetch_or(uint16_t(1u) << symbol, std::memory_order_relaxed);
    ...
    value |= uint16_t(1u) << symbol;

    if (are_all_symbols_notified(value)) {
      clear_slot(slot);
    }

    return std::make_optional<interval<unsigned>>(symbol, symbol + nof_symbols_to_notify);
  }
};
