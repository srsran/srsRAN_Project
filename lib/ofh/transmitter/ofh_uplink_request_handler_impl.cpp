...
void uplink_request_handler_impl::handle_prach_occasion(const prach_buffer_context& context, prach_buffer& buffer)
{
  ...
  cplane_scheduling_prach_context cp_prach_context = {};
  ...
  filter_index_type filter_type = get_prach_cplane_filter_index(context, preamble_info);

  for (auto eaxc : prach_eaxc) {
    data_flow->enqueue_section_type_3_prach_message(context.slot, eaxc, filter_type, cp_prach_context);
  }
}
