...
void iq_compression_bfp_avx512::decompress(span<cbf16_t>                output,
                                           ...
                                           const ru_compression_params& params)
{
  ...
  for (const auto& c_prb : input) {
    ...
    mm512::unpack_prb_big_endian(unpacked_prb_span, c_prb.get_packed_data(), params.data_width);
  ...
  span<float>   unpacked_iq_scaling_span(unpacked_iq_scaling.data(), output.size() * 2);

  // Scale unpacked IQ samples using saved exponents and convert to complex samples.
  srsvec::convert(output, unpacked_iq_int16_span, unpacked_iq_scaling_span);
}
