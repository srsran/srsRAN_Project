                                                   ...
                                                   const e1ap_bearer_context_setup_request&   request)
{
  ...
  asn1_request->security_info.value.upsecuritykey.encryption_key =
      request.security_info.upsecuritykey.encryption_key.copy();
  asn1_request->security_info.value.upsecuritykey.integrity_protection_key =
      request.security_info.upsecuritykey.integrity_protection_key.copy();

  // ue dl aggregate maximum bit rate
  asn1_request->uedl_aggregate_maximum_bit_rate.value = request.uedl_aggregate_maximum_bit_rate;
  ...
  if (request.uedl_maximum_integrity_protected_data_rate.has_value()) {
    asn1_request->uedl_maximum_integrity_protected_data_rate_present = true;
    asn1_request->uedl_maximum_integrity_protected_data_rate.value =
  ...
  if (request.ranueid.has_value()) {
    asn1_request->ranueid_present = true;
    asn1_request->ranueid.value.from_number(ran_ue_id_to_uint(request.ranueid.value()));
  }
    ...
    for (const auto& asn1_res_setup_item : asn1_bearer_context_setup_response.pdu_session_res_setup_list.value) {
      ...
      for (const auto& asn1_drb_setup_item : asn1_res_setup_item.drb_setup_list_ng_ran) {
        ...
        for (auto& asn1_qos_flow_item : asn1_drb_setup_item.flow_setup_list) {
          ...
          qos_flow_item.qos_flow_id = asn1_qos_flow_item.qo_s_flow_id;
        ...
        for (const auto& asn1_failed_qos_flow_item : asn1_drb_setup_item.flow_failed_list) {
          ...
          failed_qos_flow_item.qos_flow_id = asn1_failed_qos_flow_item.qo_s_flow_id;
