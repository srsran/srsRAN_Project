...
void handover_reconfiguration_routine::operator()(coro_context<async_task<bool>>& ctx)
{
  ...
  if (ue_context_mod_response.success) {
    CORO_AWAIT_VALUE(
        procedure_result,
        target_ue.get_rrc_ue_notifier().on_handover_reconfiguration_complete_expected(ho_reconf_ctxt.transaction_id));
  } else {
    logger.debug("source_ue={} target_ue={}: UE context modification failed",
