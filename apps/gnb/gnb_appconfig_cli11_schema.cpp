...
static void configure_cli11_pdsch_args(CLI::App& app, pdsch_appconfig& pdsch_params)
{
  ...
  app.add_option("--max_rb_size", pdsch_params.max_rb_size, "Maximum RB size for UE PDSCH resource allocation")
      ->capture_default_str()
      ->check(CLI::Range(1U, (unsigned)MAX_NOF_PRBS));
  app.add_option("--start_rb", pdsch_params.start_rb, "Start RB for resource allocation of UE PDSCHs")
      ->capture_default_str()
      ->check(CLI::Range(0U, (unsigned)MAX_NOF_PRBS));
  app.add_option("--end_rb", pdsch_params.end_rb, "End RB for resource allocation of UE PDSCHs")
      ->capture_default_str()
      ->check(CLI::Range(0U, (unsigned)MAX_NOF_PRBS));
