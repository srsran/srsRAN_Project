...
nlohmann::json generate(const ofh::metrics& metrics, span<const pci_t> pci_sector_map);

/// Generates a string in JSON format that codifies the given Open Fronthaul low metrics.
std::string generate_string(const ofh::metrics& metrics, span<const pci_t> pci_sector_map, int indent = -1);
