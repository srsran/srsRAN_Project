                                                                ...
                                                                const cu_up_unit_dependencies& dependencies)
{
  ...
  config.ctrl_executor                  = dependencies.workers->cu_up_ctrl_exec;
  config.cu_up_e2_exec                  = dependencies.workers->cu_up_e2_exec;
  config.ue_exec_pool                   = dependencies.workers->cu_up_exec_mapper.get();
  config.io_ul_executor      = dependencies.workers->cu_up_io_ul_exec; // Optionally select separate exec for UL IO
  ...
  if (not unit_cfg.upf_cfg.no_core) {
    ...
    ngu_gw =
        srs_cu_up::create_udp_ngu_gateway(ngu_gw_config, *dependencies.io_brk, *dependencies.workers->cu_up_io_ul_exec);
  } else {
