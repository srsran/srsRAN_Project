...
/// Multicell flexible O-RAN DU factory interface.
class multicell_flexible_o_du_factory
{
  const flexible_o_du_unit_config config;

public:
  explicit multicell_flexible_o_du_factory(const flexible_o_du_unit_config& config_) : config(config_) {}
  virtual ~multicell_flexible_o_du_factory() = default;

  /// Creates a multicell flexible O-RAN DU using the given dependencies and configuration.
