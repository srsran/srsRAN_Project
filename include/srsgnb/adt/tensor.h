...
template <unsigned NDIMS, typename Type>
class tensor
{
public:
  /// Default destructor.
...
template <unsigned NDIMS, typename Type, unsigned MAX_ELEMENTS>
class static_tensor : public tensor<NDIMS, Type>
{
public:
  ...
  void resize(std::array<unsigned, NDIMS> dims)
  {
    ...
    srsgnb_assert(total_size <= MAX_ELEMENTS,
                  "The total number of elements {} (dims: {}) exceeds the maximum number of elements {}.",
                  total_size,
                  span<const unsigned>(dims),
                  MAX_ELEMENTS);
  }
...
/// \tparam Type         Type of data to store.
template <unsigned NDIMS, typename Type>
class dynamic_tensor : public tensor<NDIMS, Type>
{
public:
  using dimensions_size_type = std::array<unsigned, NDIMS>;
  ...
  explicit dynamic_tensor(const dimensions_size_type& dims) { reserve(dims); }
