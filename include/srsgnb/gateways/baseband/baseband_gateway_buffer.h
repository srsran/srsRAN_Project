...
public:
  ...
  {
    SRSGNB_ALWAYS_ASSERT__(!data.empty(), "Data is empty. Was the buffer moved?");
    SRSGNB_ALWAYS_ASSERT__(new_nof_samples > 0 && new_nof_samples * nof_channels <= data.size(),
  ...
  {
    SRSGNB_ALWAYS_ASSERT__(nof_channels > 0, "Invalid number of channels ({}).", nof_channels);
    SRSGNB_ALWAYS_ASSERT__(nof_samples > 0, "Invalid maximum number of samples ({}).", nof_samples);
  }
