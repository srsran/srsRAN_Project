...
public:
  ...
  transaction() : transaction_id(std::numeric_limits<unsigned>::max()), ev(nullptr) {}
  transaction(unsigned transaction_id_, manual_event<result_type>& ev_) : transaction_id(transaction_id_), ev(&ev_) {}

  /// Gets transaction id.
  unsigned id() const { return transaction_id; }

  bool is_set() const { return ev->is_set(); }

  const T& get() { return ev->get(); }
...
private:
  ...
  friend class asn1_transaction_manager;
...
class asn1_transaction_manager
{
public:
  explicit asn1_transaction_manager(timer_manager& timer_db_, const T& cancel_value_ = {}) : timer_db(timer_db_), cancel_value(cancel_value_) {}
  ...
  transaction<T> create_transaction(unsigned time_to_cancel) {
    transaction<T> t = create_transaction();
    // Setup timeout.
