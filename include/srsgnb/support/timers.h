...
#ifndef SRSGNB_INCLUDE_SRSGNB_SUPPORT
#define SRSGNB_INCLUDE_SRSGNB_SUPPORT
...
class timer_handler
{
  using tic_difference                  = uint32_t;
  using tic_type                        = uint32_t;
  ...
  constexpr static tic_difference MAX_TIMER_DURATION = 0x3FFFFFFFU;
  ...
  static tic_difference decode_duration(uint64_t value) { return (value >> 32U) & MAX_TIMER_DURATION; }
  static tic_type       decode_timeout(uint64_t value) { return static_cast<uint32_t>(value & 0xFFFFFFFFU); }
  ...
  struct timer_impl : public intrusive_double_linked_list_element<>, public intrusive_forward_list_element<> {
    ...
    unique_function<void(uint32_t)> callback;
    ...
    tic_difference time_elapsed_() const
    {
    ...
    void set(uint32_t duration_, unique_function<void(uint32_t)> callback_)
    {
      srsran_assert(duration_ <= MAX_TIMER_DURATION,
                    "Invalid timer duration=%" PRIu32 ">%" PRIu32,
                    duration_,
...
public:
  ...
  public:
    ...
    tic_difference time_elapsed() const { return is_valid() ? handle->time_elapsed_() : 0; }
    tic_difference duration() const { return is_valid() ? handle->duration_() : 0; }
  ...
  void defer_callback(uint32_t duration, const F& func)
  {
    ...
    unique_function<void(uint32_t)> c     = [func, &timer](uint32_t tid) {
      func();
      // auto-deletes timer
      timer.deallocate();
    };
...
private:
  ...
  void dealloc_timer_(timer_impl& timer)
  {
    ...
    timer.callback = unique_function<void(uint32_t)>();
  ...
  std::atomic<tic_type> cur_time{0};
...
#endif // SRSGNB_INCLUDE_SRSGNB_SUPPORT
