...
public:
  ...
  byte_buffer_view2() = default;

  /// Conversion from a pair of iterators.
  byte_buffer_view2(iterator it_begin_, iterator it_end_) : it(it_begin_), it_end(it_end_) {}
  ...
  byte_buffer_view2(const ByteBufferType& buffer) : it(buffer.begin()), it_end(buffer.end())
  {
  }
  template <typename ByteBufferType, std::enable_if_t<is_owning_byte_buffer_type<ByteBufferType>::value, int> = 0>
  byte_buffer_view2(const ByteBufferType& buffer, size_t start, size_t sz) : it(buffer.begin() + start), it_end(it + sz)
  {
  }
  ...
  byte_buffer_view2 view(size_t offset, size_t size) const
  {
  ...
  std::pair<byte_buffer_view2, byte_buffer_view2> split(size_t offset)
  {
