...
class du_timer_source
{
  struct ticker_deleter {
    void operator()(du_timer_source* ts) const
    {
      if (ts != nullptr) {
        ts->do_reset_ticker();
      }
...
public:
  ...
  {
  public:
    explicit unique_ticker(du_timer_source& parent_) : parent(&parent_) {}

    void tick() { parent->do_tick(); }

  private:
    std::unique_ptr<du_timer_source, ticker_deleter> parent;
  };

  virtual ~du_timer_source() = default;
