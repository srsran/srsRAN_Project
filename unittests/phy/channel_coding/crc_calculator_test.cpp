...
crc_calculator_checksum_t crc_generic_calculator(const srsgnb::byte_buffer& data, unsigned poly, unsigned order)
{
...
void test_crc(std::size_t N, crc_generator_poly poly, unsigned polynom, unsigned order)
{
  std::uniform_int_distribution<unsigned char> dist(0, UINT8_MAX);

  // Create data buffer
  srsgnb::byte_buffer data(N);
  ...
  crc_calculator_checksum_t checksum = create_crc_calculator(poly)->calculate(data);
...
int main()
{
  ...
  for (std::size_t N : sizes) {
    test_crc(N, crc_generator_poly::CRC24A, 0x1864CFB, 24);
    test_crc(N, crc_generator_poly::CRC24B, 0X1800063, 24);
    test_crc(N, crc_generator_poly::CRC24C, 0X1B2B117, 24);
    test_crc(N, crc_generator_poly::CRC16, 0x11021, 16);
    test_crc(N, crc_generator_poly::CRC11, 0xE21, 11);
    //    test_crc(N, crc_generator_poly::CRC6, 0x61, 6);
  }
}