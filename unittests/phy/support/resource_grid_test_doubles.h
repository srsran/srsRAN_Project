...
class file_resource_grid_reader : public resource_grid_reader
{
  enum class dimensions : unsigned { subcarrier = 0, symbol, port, all };

public:
  file_resource_grid_reader(file_vector<cf_t> file, unsigned nof_prb, unsigned nof_symbols, unsigned nof_ports) :
    data({nof_prb * NRE, nof_symbols, nof_ports})
  {
    std::vector<cf_t> file_data = file.read();

    report_fatal_error_if_not(data.size() == file_data.size(),
                              "The number of resource elements in the grid (i.e., {}) does not match with the number "
                              "of samples contained in the file (i.e., {}).",
                              data.size(),
                              file_data.size());

    srsvec::copy(data.get_data(), file_data);
  }
  ...
  {
    for (const resource_grid_coordinate& coordinate : coordinates) {
      symbols.front() = data[{coordinate.subcarrier, coordinate.symbol, port}];
      symbols         = symbols.last(symbols.size() - 1);
    }
  }
  ...
  span<cf_t> get(span<cf_t> symbols, unsigned port, unsigned l, unsigned k_init, span<const bool> mask) const override
  {
    bounded_bitset<MAX_RB * NRE> bit_mask(mask.begin(), mask.end());
    return get(symbols, port, l, k_init, bit_mask);
  }
  ...
  span<cf_t> get(span<cf_t>                          symbols,
                 ...
                 const bounded_bitset<MAX_RB * NRE>& mask) const override
  {
    span<const cf_t> ofdm_symbol_view = data.get_view({l, port});
    mask.for_each(0, mask.size(), [&symbols, &k_init, &ofdm_symbol_view](unsigned i_subc) {
      symbols.front() = ofdm_symbol_view[i_subc + k_init];
      symbols         = symbols.last(symbols.size() - 1);
    });
    return symbols;
  }
  ...
  {
    span<const cf_t> ofdm_symbol_view = data.get_view({l, port});
    srsvec::copy(symbols, ofdm_symbol_view.subspan(k_init, symbols.size()));
  }

private:
  dynamic_tensor<std::underlying_type_t<dimensions>(dimensions::all), cf_t, dimensions> data;
};
