...
TEST_F(du_ran_resource_manager_tester, when_ue_resource_config_is_created_then_pcell_is_configured)
{
  ...
  ASSERT_TRUE(ue_res->scells.empty());
  ASSERT_TRUE(ue_res->rlc_bearers.empty());
  ASSERT_EQ(ue_res->spcell_cfg.cell_index, to_du_cell_index(0));
  ASSERT_EQ(ue_res->spcell_cfg.serv_cell_idx, SERVING_CELL_PCELL_IDX);
  ASSERT_FALSE(ue_res->spcell_cfg.spcell_cfg_ded.ul_config->init_ul_bwp.pucch_cfg->sr_res_list.empty());
...
TEST_F(du_ran_resource_manager_tester, when_multiple_ues_are_created_then_they_use_different_sr_offsets)
{
  ...
  for (unsigned i = 0; i != sr_period; ++i) {
    ...
    ASSERT_FALSE(ue_res.empty());
    const auto& sr_res_list = ue_res->spcell_cfg.spcell_cfg_ded.ul_config->init_ul_bwp.pucch_cfg->sr_res_list;
  ...
  unsigned      rem_sr_offset =
      ues[ue_idx_to_rem]->spcell_cfg.spcell_cfg_ded.ul_config->init_ul_bwp.pucch_cfg->sr_res_list[0].offset;
  ues.erase(ue_idx_to_rem);
  ue_ran_resource_configurator& ue_res = create_ue(to_du_ue_index(sr_period + 1));
  ASSERT_FALSE(ue_res.empty());
  ASSERT_EQ(rem_sr_offset, ue_res->spcell_cfg.spcell_cfg_ded.ul_config->init_ul_bwp.pucch_cfg->sr_res_list[0].offset);
}
