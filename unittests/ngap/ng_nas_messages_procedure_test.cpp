...
TEST_F(ngc_test, when_initial_context_setup_request_received_then_response_send)
{
  ASSERT_EQ(ngc->get_nof_ues(), 0);
  ...
  ngc_message init_context_setup_request = generate_initial_context_setup_request_message();
  ngc->handle_message(init_context_setup_request);

  ngap_initial_context_setup_response_message init_ctxt_resp_msg =
      generate_successful_initial_context_setup_response_message();
  ngc->handle_initial_context_setup_response_message(init_ctxt_resp_msg);
