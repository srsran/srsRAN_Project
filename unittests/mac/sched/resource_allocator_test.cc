...
void test_pusch_resource_allocation()
{
  ...
  TESTASSERT(res_alloc[0].used_ul_prbs().none());
  TESTASSERT(res_alloc[0].ul_res().puschs.empty());

  // Action 2: Allocate PUSCH grant in current slot_tx
  prb_interval ul_grant{1, 5};
  res_alloc[0].alloc_pusch(ul_grant);

  // Test: Allocated PUSCH was registered in the cell resource grid for slot_tx
  TESTASSERT(res_grid_pool[sl_tx].ul_prbs.prbs().any());
  TESTASSERT_EQ(res_grid_pool[sl_tx].ul_prbs.prbs().count(), ul_grant.length());
  ...
  res_alloc[1].alloc_pusch(ul_grant2);

  // Test: Allocated PUSCH was registered in the cell resource grid for slot_tx + 1
  TESTASSERT(res_grid_pool[sl_tx + 1].ul_prbs.prbs().any());
  TESTASSERT_EQ(res_grid_pool[sl_tx + 1].ul_prbs.prbs().count(), ul_grant2.length());
  ...
  TESTASSERT_EQ(res_grid_pool[sl_tx - 1].ul_prbs.prbs().count(), ul_grant.length());

  // Test: Current slot_tx allocations match the ones done with "slot_tx + 1" in the previous slot
  TESTASSERT_EQ(res_grid_pool[sl_tx].ul_prbs.prbs().count(), ul_grant2.length());
}
