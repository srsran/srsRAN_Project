...
TEST_F(f1ap_du_test, when_dl_rrc_message_transfer_is_received_lower_layer_srb_notifier_receives_rrc_pdu)
{
  ...
  ASSERT_TRUE(ue->f1c_bearers[0].tx_pdu_notifier.last_pdu.empty());

  // Test section
  byte_buffer test_rrc_msg = {0x1, 0x2, 0x3};
  f1c_message msg          = generate_f1_dl_rrc_message_transfer(srb_id_t::srb1, test_rrc_msg);
  f1ap->handle_message(msg);
  ASSERT_FALSE(ue->f1c_bearers[0].tx_pdu_notifier.last_pdu.empty());
  ASSERT_EQ(ue->f1c_bearers[0].tx_pdu_notifier.last_pdu, test_rrc_msg);
}
...
TEST_F(f1ap_du_test, when_dl_rrc_message_transfer_is_received_gnb_cu_ue_f1ap_id_is_updated)
{
  ...
  f1ap->handle_message(msg);
  test_ues[0].f1c_bearers[0].bearer->handle_pdu(byte_buffer_slice_chain{test_rrc_msg.copy()});
