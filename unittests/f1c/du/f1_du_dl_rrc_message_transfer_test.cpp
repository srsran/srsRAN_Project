...
TEST_F(f1ap_du_test, when_dl_rrc_message_transfer_is_received_lower_layer_srb_notifier_receives_rrc_pdu)
{
  ...
  ue_test_context* ue = run_f1_ue_create(to_du_ue_index(0));
  ASSERT_TRUE(ue->tx_pdu_notif->last_pdu.empty());

  // Test section
  byte_buffer test_rrc_msg = {0x1, 0x2, 0x3};
  f1c_message msg          = generate_f1_dl_rrc_message_transfer(test_rrc_msg);
  f1ap->handle_message(msg);
  ASSERT_FALSE(ue->tx_pdu_notif->last_pdu.empty());
  ASSERT_EQ(ue->tx_pdu_notif->last_pdu, test_rrc_msg);
}
...
TEST_F(f1ap_du_test, when_dl_rrc_message_transfer_is_received_gnb_cu_ue_f1ap_id_is_updated)
{
  ...
  f1ap->handle_message(msg);
  test_ues[0].bearers[0]->handle_pdu(byte_buffer_slice_chain{test_rrc_msg.copy()});
