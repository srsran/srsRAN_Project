...
TEST_F(f1ap_du_ue_context_setup_test, when_f1ap_receives_request_then_f1ap_responds_back_with_ue_context_setup_response)
{
  ...
  ASSERT_EQ(this->msg_notifier.last_f1c_msg.pdu.type().value, f1ap_pdu_c::types_opts::successful_outcome);
...
TEST_F(f1ap_du_ue_context_setup_test, when_f1ap_receives_request_then_new_srbs_become_active)
{
  ...
  ASSERT_EQ(this->msg_notifier.last_f1c_msg.pdu.type().value, f1ap_pdu_c::types_opts::init_msg);
  ...
  const ul_rrc_msg_transfer_s& ulmsg = this->msg_notifier.last_f1c_msg.pdu.init_msg().value.ul_rrc_msg_transfer();
